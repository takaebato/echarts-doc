...
If [matrix.x.data](#matrix.x.data) is not provided, it will be collected from `series.data` or `dataset.soruce`.

See [matrix data collection example](http://localhost/echarts-website/examples/en/editor.html?c=matrix-mini-bar-data-collection&edit=1&reset=1).
...
See [matrix.x.data](#matrix.x.data).
...
If [matrix.y.data](#matrix.y.data) is not provided, it will be collected from `series.data` or `dataset.soruce`.
...
See [matrix.y.data](#matrix.y.data).

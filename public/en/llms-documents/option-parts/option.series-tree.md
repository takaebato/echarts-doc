...
See also [series-tree.coordinateSystemUsage](#series-tree.coordinateSystemUsage).
...
*   `'box'`: **(Not applicable in [series-tree](#series-tree))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-tree.coords](#series-tree.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-tree.coordinateSystem](#series-tree.coordinateSystem).
...
When [roaming](#series-tree.roam), the values in [center](#series-tree.center) and `zoom` will be modified correspondingly.

## scaleLimit
- **Type**: `Object`

Limit of [zooming](#series-tree.roam), with `min` and `max`.
...
When roaming, the values in [center](#series-tree.center) and [zoom](#series-tree.zoom) will be modified correspondingly.

...
See also [series-sankey.coordinateSystemUsage](#series-sankey.coordinateSystemUsage).
...
*   `'box'`: **(Not applicable in [series-sankey](#series-sankey))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-sankey.coords](#series-sankey.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-sankey.coordinateSystem](#series-sankey.coordinateSystem).
...
When [roaming](#series-sankey.roam), the values in [center](#series-sankey.center) and `zoom` will be modified correspondingly.

## scaleLimit
- **Type**: `Object`

Limit of [zooming](#series-sankey.roam), with `min` and `max`.
...
When roaming, the values in [center](#series-sankey.center) and [zoom](#series-sankey.zoom) will be modified correspondingly.

...
*   `'calendar'`
    ...
    Lay out based on a [calendar coordinate system](#calendar). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](#series-chord.calendarIndex) or [calendarId](#series-chord.calendarId).
    

*   `'matrix'`
    
    Lay out based on a [matrix coordinate system](#matrix). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](#series-chord.matrixIndex) or [matrixId](#series-chord.matrixId).
...
See also [series-chord.coordinateSystemUsage](#series-chord.coordinateSystemUsage).
...
Specify how to lay out this `series-chord` based on the specified [coordinateSystem](#series-chord.coordinateSystem).
...
*   `'box'`: **(Not applicable in [series-chord](#series-chord))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-chord.coords](#series-chord.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-chord.coordinateSystem](#series-chord.coordinateSystem).
...
When [coordinateSystemUsage](#series-chord.coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](#series-chord.coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.

...
See also [USA Population Estimates](http://localhost/echarts-website/examples/en/editor.html?c=map-usa).
...
See also [Beef Cuts](http://localhost/echarts-website/examples/en/editor.html?c=geo-beef-cuts).

The demo above shows that SVG format can be used in ECharts. See more info in [SVG Base Map](http://localhost/echarts-website/handbook/en/how-to/component-types/geo/svg-base-map).
...
`center` is in longitude and latitude by default. Use the projected coordinates if [projection](#series-map.projection) is set.
...
A percentage string can also be used in `center`, like `'30%'`, based on the bounding rect(determined min/max latitude/longitude, or min/max projected coordinates if [projection](#series-map.projection) is set). You can use `'0%'` to place the top or left of bounding rect to the center of the viewport (typically, canvas), or use `'100%'` to place the right or bottom to the center of the viewport, or use `'50%'` to place the entire source map at the the center of the viewport. For example:
...
**See [geo roam indicator example](http://localhost/echarts-website/examples/en/editor.html?c=doc-example/geo-roam-indicator&edit=1&reset=1)** to understand the concept.
...
Limit of [zooming](#series-map.roam), with `min` and `max`.
...
When roaming, the values in [center](#series-map.center) and [zoom](#series-map.zoom) will be modified correspondingly.
...
Used to scale aspect of geo. It will be ignored if [projection](#series-map.projection) is set.

The final calculated `pixelWidth` and `pixelHeight` of the map will satisfy `pixelWidth / pixelHeight = lngSpan / latSpan * aspectScale` (assume [projection](#series-map.projection) is not specified, and [preserveAspect](#series-map.preserveAspect) is truthy).

If no [projection](#series-map.projection) is applied, the latitudes and longitudes in GeoJSON are linearly mapped to pixel coordinates diarectly. `aspectScale` offers a simple way to visually compensates for the distortion caused by the fact that the longitudinal spacing shrinks as latitude increases. For example, an `aspectScale` can be roughly calculated as `aspectScale = Math.cos(center_latitude * Maht.PI / 180)`, which is similar to a sinusoidal projection.

See [example](http://localhost/echarts-website/examples/en/editor.html?c=geo-graph&edit=1&reset=1).
...
> *   [series-map.layoutCenter](#series-map.layoutCenter) / [.layoutSize](#series-map.layoutSize)
...
**See example:** [geo roam indicator](http://localhost/echarts-website/examples/en/editor.html?c=doc-example/geo-roam-indicator&edit=1&reset=1).
...
See also [series-map.coordinateSystemUsage](#series-map.coordinateSystemUsage).
...
*   `'box'`: **(Not applicable in [series-map](#series-map))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-map.coords](#series-map.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-map.coordinateSystem](#series-map.coordinateSystem).
...
The first parameter `value` is the value in [data](#series-map.data), and the second parameter `params` is the rest parameters of data item.
...
1.  Assign coordinate according to container with [x](#series-map.markPoint.data.x), [y](#series-map.markPoint.data.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](#series-map.markPoint.data.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](#series-map.markPoint.data.label.formatter).

#### markPoint.data.valueIndex
- **Type**: `number`

Available when using [type](#series-map.markPoint.data.type) it is used to assign maximum value and minimum value in dimensions, it could be `0` (xAxis, radiusAxis), `1` (yAxis, angleAxis), and use the first value axis dimension by default.

#### markPoint.data.valueDim
- **Type**: `string`

Works only when [type](#series-map.markPoint.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](#series-map.markPoint.data.x) and [y](#series-map.markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](#series-map.markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](#series-map.markLine.data.0.x), [y](#series-map.markLine.data.0.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](#series-map.markLine.data.0.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
...
Works only when [type](#series-map.markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be `0` (for xAxis, or radiusAxis), or `1` (for yAxis, or angleAxis). Dimension of the first numeric axis is used by default.

##### markLine.data.0.valueDim
- **Type**: `string`

Works only when [type](#series-map.markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](#series-map.markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](#series-map.markArea.data.0.x), [y](#series-map.markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).

2.  Specify the coordinate in data coordinate system (i.e., cartesian) using [coord](#series-map.markArea.data.0.coord), which can be also set as `'min'`, `'max'`, `'average'` (e.g, `coord: [23, 'min']`, or `coord: ['average', 'max']`).
...
Specify the dimension on which min, max, average are calculated, available when [type](#series-map.markArea.data.type) used. The value can be `0` (means xAxis, radiusAxis) or `1` (means yAxis, angleAxis), using the dimension of the first axis by default.

##### markArea.data.0.valueDim
- **Type**: `string`

Specify the dimension on which min, max, average are calculated, available when [type](#series-map.markArea.data.type) used. The value can be the name of the dimension (for example, the value can be `x`, `angle` in line chart, and `open`, `close` in candlestick).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](#series-map.markArea.data.label.formatter).

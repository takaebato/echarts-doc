...
See also [series-lines.coordinateSystemUsage](#series-lines.coordinateSystemUsage).
...
*   `'box'`: **(Not applicable in [series-lines](#series-lines))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-lines.coords](#series-lines.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-lines.coordinateSystem](#series-lines.coordinateSystem).
...
If it is set true, [data.coords](#series-lines.data.coords) can have more than two coord to draw a polyline. It is useful when visualizing GPS track data. See example [lines-bus](http://localhost/echarts-website/examples/en/editor.html?c=lines-bmap-bus).
...
Line color. Color is taken from [option.color Palette](#color) by default.
...
The first parameter `value` is the value in [data](#series-lines.data), and the second parameter `params` is the rest parameters of data item.
...
1.  Assign coordinate according to container with [x](#series-lines.markPoint.data.x), [y](#series-lines.markPoint.data.y) attribute, in which pixel values and percentage are supported.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](#series-lines.markPoint.data.label.formatter).
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](#series-lines.markPoint.data.x) and [y](#series-lines.markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](#series-lines.markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](#series-lines.markLine.data.0.x), [y](#series-lines.markLine.data.0.y) attribute, in which pixel values and percentage are supported.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](#series-lines.markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](#series-lines.markArea.data.0.x), [y](#series-lines.markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](#series-lines.markArea.data.label.formatter).

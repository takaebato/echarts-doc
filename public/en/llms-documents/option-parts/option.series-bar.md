...
See also [series-bar.coordinateSystemUsage](#series-bar.coordinateSystemUsage).
...
*   `'box'`: **(Not applicable in [series-bar](#series-bar))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-bar.coords](#series-bar.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-bar.coordinateSystem](#series-bar.coordinateSystem).
...
Whether to enable realtime sorting, which is used for bar-racing effect. Please refer to the tutorial [Dynamic Sorting Bar Chart](http://localhost/echarts-website/handbook/en/how-to/chart-types/bar/bar-race/) in the Handbook.
...
Bar color. By default, colors from global palette [option.color](#color) is used.
...
The first parameter `value` is the value in [data](#series-bar.data), and the second parameter `params` is the rest parameters of data item.
...
1.  Assign coordinate according to container with [x](#series-bar.markPoint.data.x), [y](#series-bar.markPoint.data.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](#series-bar.markPoint.data.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
    
3.  Use [type](#series-bar.markPoint.data.type) attribute to mark the maximum and minimum values in the series, in which [valueIndex](#series-bar.markPoint.data.valueIndex) or [valueDim](#series-bar.markPoint.data.valueDim) can be used to assign the dimension.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](#series-bar.markPoint.data.label.formatter).
...
Available when using [type](#series-bar.markPoint.data.type) it is used to assign maximum value and minimum value in dimensions, it could be `0` (xAxis, radiusAxis), `1` (yAxis, angleAxis), and use the first value axis dimension by default.

#### markPoint.data.valueDim
- **Type**: `string`

Works only when [type](#series-bar.markPoint.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](#series-bar.markPoint.data.x) and [y](#series-bar.markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](#series-bar.markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](#series-bar.markLine.data.0.x), [y](#series-bar.markLine.data.0.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](#series-bar.markLine.data.0.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
    
3.  Use [type](#series-bar.markLine.data.0.type) attribute to mark the maximum and minimum values in the series, in which [valueIndex](#series-bar.markLine.data.0.valueIndex) or [valueDim](#series-bar.markLine.data.0.valueDim) can be used to assign the dimension.
...
Works only when [type](#series-bar.markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be `0` (for xAxis, or radiusAxis), or `1` (for yAxis, or angleAxis). Dimension of the first numeric axis is used by default.

##### markLine.data.0.valueDim
- **Type**: `string`

Works only when [type](#series-bar.markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](#series-bar.markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](#series-bar.markArea.data.0.x), [y](#series-bar.markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).

2.  Specify the coordinate in data coordinate system (i.e., cartesian) using [coord](#series-bar.markArea.data.0.coord), which can be also set as `'min'`, `'max'`, `'average'` (e.g, `coord: [23, 'min']`, or `coord: ['average', 'max']`).
    
3.  Locate the point on the min value or max value of `series.data` using [type](#series-bar.markArea.data.0.type), where [valueIndex](#series-bar.markArea.data.0.valueIndex) or [valueDim](#series-bar.markPoint.data.0.valueDim) can be used to specify the dimension on which the min, max or average are calculated.
...
Specify the dimension on which min, max, average are calculated, available when [type](#series-bar.markArea.data.type) used. The value can be `0` (means xAxis, radiusAxis) or `1` (means yAxis, angleAxis), using the dimension of the first axis by default.

##### markArea.data.0.valueDim
- **Type**: `string`

Specify the dimension on which min, max, average are calculated, available when [type](#series-bar.markArea.data.type) used. The value can be the name of the dimension (for example, the value can be `x`, `angle` in line chart, and `open`, `close` in candlestick).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](#series-bar.markArea.data.label.formatter).

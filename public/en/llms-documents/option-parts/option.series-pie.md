...
*   `'geo'`
    ...
    See example [pie in geo](http://localhost/echarts-website/examples/en/editor.html?c=map-iceland-pie&edit=1&reset=1).
...
See also [series-pie.coordinateSystemUsage](#series-pie.coordinateSystemUsage).
...
*   `'box'`: **(Not applicable in [series-pie](#series-pie))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-pie.coords](#series-pie.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-pie.coordinateSystem](#series-pie.coordinateSystem).
...
[example: pie in geo](http://localhost/echarts-website/examples/en/editor.html?c=map-iceland-pie&edit=1&reset=1)
...
*   `'center'`
    ...
    In the center of pie chart. See [pie-doughnut example](http://localhost/echarts-website/examples/en/editor.html?c=pie-doughnut)
...
The first parameter `value` is the value in [data](#series-pie.data), and the second parameter `params` is the rest parameters of data item.
...
1.  Assign coordinate according to container with [x](#series-pie.markPoint.data.x), [y](#series-pie.markPoint.data.y) attribute, in which pixel values and percentage are supported.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](#series-pie.markPoint.data.label.formatter).
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](#series-pie.markPoint.data.x) and [y](#series-pie.markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](#series-pie.markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](#series-pie.markLine.data.0.x), [y](#series-pie.markLine.data.0.y) attribute, in which pixel values and percentage are supported.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](#series-pie.markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](#series-pie.markArea.data.0.x), [y](#series-pie.markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](#series-pie.markArea.data.label.formatter).

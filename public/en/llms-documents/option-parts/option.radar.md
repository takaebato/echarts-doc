...
See also [radar.coordinateSystemUsage](#radar.coordinateSystemUsage).
...
*   `'box'`:
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [radar.coords](#radar.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [radar.coordinateSystem](#radar.coordinateSystem).
...
**For a time axis ([`.type: 'time'`](#.type))**

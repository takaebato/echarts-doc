...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](#series.data) or [dataset.source](#dataset.source), or can be specified via [radiusAxis.data](#radiusAxis.data).
...
Color of axis name uses [axisLine.lineStyle.color](#radiusAxis.axisLine.lineStyle.color) by default.
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](#radiusAxis.axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](#radiusAxis.axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](#radiusAxis.min) and [max](#radiusAxis.max). **Example:**
...
It is available only in numerical axis, i.e., [type](#radiusAxis.type): 'value'.
...
This configuration item is unavailable when the [min](#radiusAxis.min) and [max](#radiusAxis.max) are set.
...
It is available only for axis of [type](#radiusAxis.type) 'value' or 'time'.
...
This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](#radiusAxis.type): 'log' axis.

## logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](#radiusAxis.type): 'log'.
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](#radiusAxis.axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](#radiusAxis.axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](#radiusAxis.axisLine.lineStyle.color)。
...
**For a time axis ([`radiusAxis.type: 'time'`](#radiusAxis.type))**
...
Color of axis label is set to be [axisLine.lineStyle.color](#radiusAxis.axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Interval of Axis splitLine, which is available in category axis. is set to be the same as [axisLabel.interval](#radiusAxis.axisLabel.interval) by default.
...
Minor split lines of axis in the [grid](#grid) area。It will align to the [minorTick](#radiusAxis.minorTick)
...
Interval of Axis splitArea, which is available in category axis. is set to be the same as [axisLabel.interval](#radiusAxis.axisLabel.interval) by default.
...
Category data, available in [type](#radiusAxis.type): 'category' axis.
...
The tooltip configuration for radiusAxis, must set [triggerEvent](#radiusAxis.triggerEvent) as `true` and enable global [option.tooltip](#tooltip) component.

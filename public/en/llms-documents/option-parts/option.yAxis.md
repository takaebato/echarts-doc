...
Offset of y axis relative to default position. Useful when multiple y axis has same [position](#yAxis.position) value. Notice: Set `yAxis.axisLine.onZero` to `false` to activate this option.
...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](#series.data) or [dataset.source](#dataset.source), or can be specified via [yAxis.data](#yAxis.data).
...
Color of axis name uses [axisLine.lineStyle.color](#yAxis.axisLine.lineStyle.color) by default.
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](#yAxis.axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](#yAxis.axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](#yAxis.min) and [max](#yAxis.max). **Example:**
...
It is available only in numerical axis, i.e., [type](#yAxis.type): 'value'.
...
This configuration item is unavailable when the [min](#yAxis.min) and [max](#yAxis.max) are set.
...
It is available only for axis of [type](#yAxis.type) 'value' or 'time'.
...
This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](#yAxis.type): 'log' axis.

## logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](#yAxis.type): 'log'.
...
Whether allow overlaping with [jitter](#yAxis.jitter). If `false`, it will try to avoid overlap. But in extreme cases, some scatters may also overlap if there is no way to avoid. The following is the effect of seeting it to be `false`:
...
When setting [jitter](#yAxis.jitter) and [jitterOverlap](#yAxis.jitterOverlap) is `false`, the minimum distance between two scatters.
...
**Other examples:** [bar-breaks-simple](http://localhost/echarts-website/examples/en/editor.html?c=bar-breaks-simple&edit=1&reset=1), [line-fisheye-lens](http://localhost/echarts-website/examples/en/editor.html?c=line-fisheye-lens&edit=1&reset=1)
...
If you import ECharts by [only importing the necessary components](http://localhost/echarts-website/handbook/en/basics/import), you need to import and register the feature `AxisBreak` explicitly. For example,
...
*   If [axis.type](#yAxis.type) is `'value'` or `'log'`, use `number` type values.
    
*   If [axis.type](#yAxis.type) is `'category'`: not supported yet.
    
*   If [axis.type](#yAxis.type) is `'time'`, the value can be:
...
Note: [yAxis.breaks.start](#yAxis.breaks.start) and [yAxis.breaks.end](#yAxis.breaks.end) are the unique identifiers for each break item. When calling [chart.setOption](api.html#echartsInstance.setOption) to modify [yAxis.breaks.gap](#yAxis.breaks.gap) or [yAxis.breaks.isExpanded](#yAxis.breaks.isExpanded), `start` and `end` must be specified. Update animations will only occur if `start` and `end` are not modified; no animation will occur if they are changed.
...
*   Percentage (string):
    ...
    Specifies the proportion relative to the axis. For example, `'5%'` means that the final size of the axis break area will always be `'5%'` of the axis length. Using a percentage ensures that the pixel size of the axis break area remains stable, and does not change when [yAxis.min](#yAxis.min), [yAxis.max](#yAxis.max), or [dataZoom](#dataZoom) are modified. For this reason, using a percentage is recommended in most scenarios.
    
*   Absolute value:
    
    Its unit is the same as [start](#yAxis.breaks.start) and [end](#yAxis.breaks.end), referring to a value in the data domain defined by the business data (`series.data`), rather than pixels. It represents mapping (replacing) the `[start, end]` interval with `[start, start + gap]`. Therefore, if set as an absolute value, the pixel size of the axis break area will change when [yAxis.min](#yAxis.min), [yAxis.max](#yAxis.max), or [dataZoom](#dataZoom) are modified.
    

**Notice:** Within a [yAxis.breaks](#yAxis.breaks) array, `gap` must be specified either entirely in percentages or entirely in absolute values. Mixing the two is not allowed, as it may lead to unexpected results.
...
See also the introduction to the axis break in [yAxis.breaks](#yAxisbreaks).
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](#yAxis.axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](#yAxis.axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](#yAxis.axisLine.lineStyle.color)。
...
**For a time axis ([`yAxis.type: 'time'`](#yAxis.type))**
...
Vertical alignment of the label of the min tick. If set to be `null`, it's the same with other labels. See [verticalAlign](#yAxis.axisLabel.verticalAlign).
...
Vertical alignment of the label of the max tick. If set to be `null`, it's the same with other labels. See [verticalAlign](#yAxis.axisLabel.verticalAlign).
...
Color of axis label is set to be [axisLine.lineStyle.color](#yAxis.axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Interval of Axis splitLine, which is available in category axis. is set to be the same as [axisLabel.interval](#yAxis.axisLabel.interval) by default.
...
Minor split lines of axis in the [grid](#grid) area。It will align to the [minorTick](#yAxis.minorTick)
...
Interval of Axis splitArea, which is available in category axis. is set to be the same as [axisLabel.interval](#yAxis.axisLabel.interval) by default.
...
Category data, available in [type](#yAxis.type): 'category' axis.
...
A button used to drag axisPointer. This feature is applicable in touch device. See [example](http://localhost/echarts-website/examples/en/editor.html?c=line-tooltip-touch&edit=1&reset=1).
...
See the [example of using image](http://localhost/echarts-website/examples/en/editor.html?c=doc-example/axisPointer-handle-image&edit=1&reset=1)
...
The tooltip configuration for yAxis, must set [triggerEvent](#yAxis.triggerEvent) as `true` and enable global [option.tooltip](#tooltip) component.

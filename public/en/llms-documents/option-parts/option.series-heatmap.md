...
See also [series-heatmap.coordinateSystemUsage](#series-heatmap.coordinateSystemUsage).
...
*   `'box'`: **(Not applicable in [series-heatmap](#series-heatmap))**
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-heatmap.coords](#series-heatmap.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-heatmap.coordinateSystem](#series-heatmap.coordinateSystem).
...
The first parameter `value` is the value in [data](#series-heatmap.data), and the second parameter `params` is the rest parameters of data item.
...
1.  Assign coordinate according to container with [x](#series-heatmap.markPoint.data.x), [y](#series-heatmap.markPoint.data.y) attribute, in which pixel values and percentage are supported.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](#series-heatmap.markPoint.data.label.formatter).
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](#series-heatmap.markPoint.data.x) and [y](#series-heatmap.markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](#series-heatmap.markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](#series-heatmap.markLine.data.0.x), [y](#series-heatmap.markLine.data.0.y) attribute, in which pixel values and percentage are supported.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](#series-heatmap.markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](#series-heatmap.markArea.data.0.x), [y](#series-heatmap.markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](#series-heatmap.markArea.data.label.formatter).

...
See also [parallel.coordinateSystemUsage](#parallel.coordinateSystemUsage).
...
*   `'box'`:
    ...
    *   For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [parallel.coords](#parallel.coords) in that system. See example [sparkline in matrix](http://localhost/echarts-website/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [parallel.coordinateSystem](#parallel.coordinateSystem).
...
[See the sample](http://localhost/echarts-website/examples/en/editor.html?c=doc-example/parallel-all&edit=1&reset=1).
...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](#series.data) or [dataset.source](#dataset.source), or can be specified via [parallelAxis.data](#parallelAxis.data).
...
Color of axis name uses [axisLine.lineStyle.color](#parallelAxis.axisLine.lineStyle.color) by default.
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](#parallelAxis.axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](#parallelAxis.axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](#parallelAxis.min) and [max](#parallelAxis.max). **Example:**
...
It is available only in numerical axis, i.e., [type](#parallelAxis.type): 'value'.
...
This configuration item is unavailable when the [min](#parallelAxis.min) and [max](#parallelAxis.max) are set.
...
It is available only for axis of [type](#parallelAxis.type) 'value' or 'time'.
...
This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](#parallelAxis.type): 'log' axis.

### parallelAxisDefault.logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](#parallelAxis.type): 'log'.
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](#parallelAxis.axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](#parallelAxis.axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](#parallelAxis.axisLine.lineStyle.color)。
...
**For a time axis ([`parallelAxis.type: 'time'`](#parallelAxis.type))**
...
Color of axis label is set to be [axisLine.lineStyle.color](#parallelAxis.axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Category data, available in [type](#parallelAxis.type): 'category' axis.
...
The tooltip configuration for parallelAxis, must set [triggerEvent](#parallelAxis.triggerEvent) as `true` and enable global [option.tooltip](#tooltip) component.

...
Offset of x axis relative to default position. Useful when multiple x axis has same [position](#xAxis.position) value. Notice: Set `xAxis.axisLine.onZero` to `false` to activate this option.
...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](#series.data) or [dataset.source](#dataset.source), or can be specified via [xAxis.data](#xAxis.data).
...
Color of axis name uses [axisLine.lineStyle.color](#xAxis.axisLine.lineStyle.color) by default.
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](#xAxis.axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](#xAxis.axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](#xAxis.min) and [max](#xAxis.max). **Example:**
...
It is available only in numerical axis, i.e., [type](#xAxis.type): 'value'.
...
This configuration item is unavailable when the [min](#xAxis.min) and [max](#xAxis.max) are set.
...
It is available only for axis of [type](#xAxis.type) 'value' or 'time'.
...
This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](#xAxis.type): 'log' axis.

## logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](#xAxis.type): 'log'.
...
Whether allow overlaping with [jitter](#xAxis.jitter). If `false`, it will try to avoid overlap. But in extreme cases, some scatters may also overlap if there is no way to avoid. The following is the effect of seeting it to be `false`:
...
When setting [jitter](#xAxis.jitter) and [jitterOverlap](#xAxis.jitterOverlap) is `false`, the minimum distance between two scatters.
...
*   If [axis.type](#xAxis.type) is `'value'` or `'log'`, use `number` type values.
    
*   If [axis.type](#xAxis.type) is `'category'`: not supported yet.
    
*   If [axis.type](#xAxis.type) is `'time'`, the value can be:
...
Note: [xAxis.breaks.start](#xAxis.breaks.start) and [xAxis.breaks.end](#xAxis.breaks.end) are the unique identifiers for each break item. When calling [chart.setOption](api.html#echartsInstance.setOption) to modify [xAxis.breaks.gap](#xAxis.breaks.gap) or [xAxis.breaks.isExpanded](#xAxis.breaks.isExpanded), `start` and `end` must be specified. Update animations will only occur if `start` and `end` are not modified; no animation will occur if they are changed.
...
*   Percentage (string):
    ...
    Specifies the proportion relative to the axis. For example, `'5%'` means that the final size of the axis break area will always be `'5%'` of the axis length. Using a percentage ensures that the pixel size of the axis break area remains stable, and does not change when [xAxis.min](#xAxis.min), [xAxis.max](#xAxis.max), or [dataZoom](#dataZoom) are modified. For this reason, using a percentage is recommended in most scenarios.
    
*   Absolute value:
    
    Its unit is the same as [start](#xAxis.breaks.start) and [end](#xAxis.breaks.end), referring to a value in the data domain defined by the business data (`series.data`), rather than pixels. It represents mapping (replacing) the `[start, end]` interval with `[start, start + gap]`. Therefore, if set as an absolute value, the pixel size of the axis break area will change when [xAxis.min](#xAxis.min), [xAxis.max](#xAxis.max), or [dataZoom](#dataZoom) are modified.
    

**Notice:** Within a [xAxis.breaks](#xAxis.breaks) array, `gap` must be specified either entirely in percentages or entirely in absolute values. Mixing the two is not allowed, as it may lead to unexpected results.
...
See also the introduction to the axis break in [xAxis.breaks](#xAxisbreaks).
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](#xAxis.axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](#xAxis.axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](#xAxis.axisLine.lineStyle.color)。
...
**For a time axis ([`xAxis.type: 'time'`](#xAxis.type))**
...
Alignment of the label of the max tick. If set to be `null`, it's the same with other labels. See [align](#xAxis.axisLabel.align).
...
Color of axis label is set to be [axisLine.lineStyle.color](#xAxis.axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Interval of Axis splitLine, which is available in category axis. is set to be the same as [axisLabel.interval](#xAxis.axisLabel.interval) by default.
...
Minor split lines of axis in the [grid](#grid) area。It will align to the [minorTick](#xAxis.minorTick)
...
Interval of Axis splitArea, which is available in category axis. is set to be the same as [axisLabel.interval](#xAxis.axisLabel.interval) by default.
...
Category data, available in [type](#xAxis.type): 'category' axis.
...
The tooltip configuration for xAxis, must set [triggerEvent](#xAxis.triggerEvent) as `true` and enable global [option.tooltip](#tooltip) component.

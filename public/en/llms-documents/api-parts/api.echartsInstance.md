...
*   In [geo](option.html#geo):
    ...
    It can also be a `string`, indicating a `name` in `GeoJSON` (i.e., `features[i].properties.name`, see details in [registerMap](#echarts.registerMap)) or `SVG` (i.e., see "Named Element" in [SVG Base Map](http://localhost/echarts-website/handbook/en/how-to/component-types/geo/svg-base-map)), and return the pixel point of the center of that area.

...
*   `'cartesian3D'`
    ...
    使用三维笛卡尔坐标系，通过 [grid3DIndex](#series-.grid3DIndex) 指定相应的三维笛卡尔坐标系组件。
    

*   `'geo3D'`
    
    使用三维地理坐标系，通过 [geo3DIndex](#series-.geo3DIndex) 指定相应的三维地理坐标系组件
    

*   `'globe'`
    
    使用地球坐标系，通过 [globeIndex](#series-.globeIndex) 指定相应的地球坐标系组件
...
真实感材质相关的配置项，在 [shading](#series-bar3D.shading) 为`'realistic'`时有效。
...
跟 [roughness](#series-bar3D.realisticMaterial.roughness) 一样 你可以直接将 `metalness` 设置为金属度贴图。
...
lambert 材质相关的配置项，在 [shading](#series-bar3D.shading) 为`'lambert'`时有效。
...
color 材质相关的配置项，在 [shading](#series-bar3D.shading) 为`'color'`时有效。

...
坐标轴名称的颜色，默认取 [axisLine.lineStyle.color](#yAxis3D.axisLine.lineStyle.color)。
...
*   `'category'` 类目轴，适用于离散的类目数据，为该类型时必须通过 [data](#yAxis3D.data) 设置类目数据。
...
只在数值轴中（[type](#yAxis3D.type): 'value'）有效。

是否是脱离 0 值比例。设置成 `true` 后坐标刻度不会强制包含零刻度。在双数值轴的散点图中比较有用。

在设置 [min](#yAxis3D.min) 和 [max](#yAxis3D.max) 之后该配置项无效。
...
因为 [splitNumber](#yAxis3D.splitNumber) 是预估的值，实际根据策略计算出来的刻度可能无法达到想要的效果，这时候可以使用 interval 配合 [min](#yAxis3D.min)、[max](#yAxis3D.max) 强制设定刻度划分，一般不建议使用。

无法在类目轴中使用。在时间轴（[type](#yAxis3D.type): 'time'）中需要传时间戳，在对数轴（[type](#yAxis3D.type): 'log'）中需要传指数值。

## logBase
- **Type**: `number`
- **Default**: `10`

对数轴的底数，只在对数轴中（[type](#yAxis3D.type): 'log'）有效。

## data
- **Type**: `Array`

类目数据，在类目轴（[type](#yAxis3D.type): 'category'）中有效。

如果设置了 [type](#yAxis3D.type) 是 `'category'`，但没有设置 `axis.data`，则 `axis.data` 的内容会自动从 [series.data](#series.data) 中获取，这会比较方便。不过注意，`axis.data` 指明的是 `'category'` 轴的取值范围。如果不指定而是从 [series.data](#series.data) 中获取，那么只能获取到 [series.data](#series.data) 中出现的值。比如说，假如 [series.data](#series.data) 为空时，就什么也获取不到。

...
真实感材质相关的配置项，在 [shading](#mapbox3D.shading) 为`'realistic'`时有效。
...
跟 [roughness](#mapbox3D.realisticMaterial.roughness) 一样 你可以直接将 `metalness` 设置为金属度贴图。
...
lambert 材质相关的配置项，在 [shading](#mapbox3D.shading) 为`'lambert'`时有效。
...
color 材质相关的配置项，在 [shading](#mapbox3D.shading) 为`'color'`时有效。
...
光照相关的设置。在 [shading](#mapbox3D.shading) 为 `'color'` 的时候无效。
...
主光源绕 x 轴，即上下旋转的角度。配合 [beta](#mapbox3D.light.main.beta) 控制光源的方向。
...
ambientCubemap 会使用纹理作为环境光的光源，会为物体提供漫反射和高光反射。可以通过 [diffuseIntensity](#mapbox3D.light.ambientCubemap.diffuseIntensity) 和 [specularIntensity](#mapbox3D.light.ambientCubemap.specularIntensity) 分别设置漫反射强度和高光反射强度。
...
注意在开启 postEffect 的时候默认会开启 [temporalSuperSampling](#mapbox3D.temporalSuperSampling) 在画面静止后持续对画面增强，包括抗锯齿、景深、SSAO、阴影等。
...
同 [screenSpaceAmbientOcclusion](#mapbox3D.postEffect.screenSpaceAmbientOcclusion)
...
在开启 [postEffect](#mapbox3D.postEffect) 后，WebGL 默认的 MSAA (Multi Sampling Anti Aliasing) 会无法使用。这时候通过 FXAA (Fast Approximate Anti-Aliasing) 可以廉价方便的解决抗锯齿的问题，FXAA 会对一些场景的边缘部分进行模糊从而解决锯齿的问题，这在一些场景上效果还不错，但是在 echarts-gl 中，需要保证很多文字和线条边缘的锐利清晰，因此 FXAA 并不是那么适用。这时候我们可以通过设置更高的`devicePixelRatio`来使用超采样，如下所示：
...
但是设置更高的`devicePixelRatio` 对电脑性能有很高的要求，所以更多时候我们建议使用 echarts-gl 中的 [temporalSuperSampling](#mapbox3D.temporalSuperSampling)，在画面静止后会持续分帧对一个像素多次抖动采样，从而达到超采样抗锯齿的效果。
...
分帧超采样。在开启 [postEffect](#mapbox3D.postEffect) 后，WebGL 默认的 MSAA 会无法使用，所以我们需要自己解决锯齿的问题。

分帧超采样是用来解决锯齿问题的方法，它在画面静止后会持续分帧对一个像素多次抖动采样，从而达到抗锯齿的效果。而且在这个分帧采样的过程中，echarts-gl 也会对 [postEffect](#mapbox3D.postEffect) 中一些需要采样保证效果的特效，例如 [SSAO](#mapbox3D.postEffect.SSAO), [景深](#mapbox3D.postEffect.depthOfField)，以及阴影进行渐进增强。
...
是否开启分帧超采样。默认在开启 [postEffect](#mapbox3D.postEffect) 后也会同步开启。

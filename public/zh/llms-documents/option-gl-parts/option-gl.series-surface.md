...
真实感材质相关的配置项，在 [shading](#series-surface.shading) 为`'realistic'`时有效。
...
跟 [roughness](#series-surface.realisticMaterial.roughness) 一样 你可以直接将 `metalness` 设置为金属度贴图。
...
lambert 材质相关的配置项，在 [shading](#series-surface.shading) 为`'lambert'`时有效。
...
color 材质相关的配置项，在 [shading](#series-surface.shading) 为`'color'`时有效。

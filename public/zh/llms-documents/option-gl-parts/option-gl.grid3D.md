...
三维笛卡尔坐标系在三维场景中的宽度。配合 [viewControl.distance](#grid3D.viewControl.distance) 可以得到最合适的展示尺寸。
...
坐标轴刻度标签的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](#.axisLabel.interval) 一样。
...
刻度线的颜色，默认取 [axisLine.lineStyle.color](#.axisLine.lineStyle.color)。
...
坐标轴分隔区域的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](#.axisLabel.interval) 一样。
...
环境贴图。支持纯色、渐变色、全景贴图的 url。默认为 `'auto'`，在配置有 [light.ambientCubemap.texture](#grid3D.light.ambientCubemap.texture) 的时候会使用该纹理作为环境贴图。否则则不显示环境贴图。
...
光照相关的设置。在 [shading](#grid3D.shading) 为 `'color'` 的时候无效。
...
主光源绕 x 轴，即上下旋转的角度。配合 [beta](#grid3D.light.main.beta) 控制光源的方向。
...
ambientCubemap 会使用纹理作为环境光的光源，会为物体提供漫反射和高光反射。可以通过 [diffuseIntensity](#grid3D.light.ambientCubemap.diffuseIntensity) 和 [specularIntensity](#grid3D.light.ambientCubemap.specularIntensity) 分别设置漫反射强度和高光反射强度。
...
注意在开启 postEffect 的时候默认会开启 [temporalSuperSampling](#grid3D.temporalSuperSampling) 在画面静止后持续对画面增强，包括抗锯齿、景深、SSAO、阴影等。
...
同 [screenSpaceAmbientOcclusion](#grid3D.postEffect.screenSpaceAmbientOcclusion)
...
在开启 [postEffect](#grid3D.postEffect) 后，WebGL 默认的 MSAA (Multi Sampling Anti Aliasing) 会无法使用。这时候通过 FXAA (Fast Approximate Anti-Aliasing) 可以廉价方便的解决抗锯齿的问题，FXAA 会对一些场景的边缘部分进行模糊从而解决锯齿的问题，这在一些场景上效果还不错，但是在 echarts-gl 中，需要保证很多文字和线条边缘的锐利清晰，因此 FXAA 并不是那么适用。这时候我们可以通过设置更高的`devicePixelRatio`来使用超采样，如下所示：
...
但是设置更高的`devicePixelRatio` 对电脑性能有很高的要求，所以更多时候我们建议使用 echarts-gl 中的 [temporalSuperSampling](#grid3D.temporalSuperSampling)，在画面静止后会持续分帧对一个像素多次抖动采样，从而达到超采样抗锯齿的效果。
...
分帧超采样。在开启 [postEffect](#grid3D.postEffect) 后，WebGL 默认的 MSAA 会无法使用，所以我们需要自己解决锯齿的问题。

分帧超采样是用来解决锯齿问题的方法，它在画面静止后会持续分帧对一个像素多次抖动采样，从而达到抗锯齿的效果。而且在这个分帧采样的过程中，echarts-gl 也会对 [postEffect](#grid3D.postEffect) 中一些需要采样保证效果的特效，例如 [SSAO](#grid3D.postEffect.SSAO), [景深](#grid3D.postEffect.depthOfField)，以及阴影进行渐进增强。
...
是否开启分帧超采样。默认在开启 [postEffect](#grid3D.postEffect) 后也会同步开启。
...
在鼠标静止操作后恢复自动旋转的时间间隔。在开启 [autoRotate](#grid3D.viewControl.autoRotate) 后有效。
...
默认视角距离主体的距离，对于 [globe](#globe) 来说是距离地球表面的距离，对于 [grid3D](#grid3D) 和 [geo3D](#geo3D) 等其它组件来说是距离中心原点的距离。在 [projection](#grid3D.viewControl.projection) 为`'perspective'`的时候有效。

### viewControl.minDistance
- **Type**: `number`
- **Default**: `40`

视角通过鼠标控制能拉近到主体的最小距离。在 [projection](#grid3D.viewControl.projection) 为`'perspective'`的时候有效。

### viewControl.maxDistance
- **Type**: `number`
- **Default**: `400`

视角通过鼠标控制能拉远到主体的最大距离。在 [projection](#grid3D.viewControl.projection) 为`'perspective'`的时候有效。

### viewControl.orthographicSize
- **Type**: `number`
- **Default**: `200`

正交投影的大小。在 [projection](#grid3D.viewControl.projection) 为`'orthographic'`的时候有效。

### viewControl.maxOrthographicSize
- **Type**: `number`
- **Default**: `40`

正交投影缩放的最大值。在 [projection](#grid3D.viewControl.projection) 为`'orthographic'`的时候有效。

### viewControl.minOrthographicSize
- **Type**: `number`
- **Default**: `400`

正交投影缩放的最小值。在 [projection](#grid3D.viewControl.projection) 为`'orthographic'`的时候有效。

### viewControl.alpha
- **Type**: `number`
- **Default**: `20`

视角绕 x 轴，即上下旋转的角度。配合 [beta](#grid3D.light.main.beta) 可以控制视角的方向。

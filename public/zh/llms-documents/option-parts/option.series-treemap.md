...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#series-treemap.calendarIndex) 或 [calendarId](#series-treemap.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#series-treemap.matrixIndex) 或 [matrixId](#series-treemap.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-treemap.coordinateSystemUsage](#series-treemap.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-treemap.coordinateSystem) 上布局本 `series-treemap`。
...
*   `'box'`：**（不适用于 [series-treemap](#series-treemap)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-treemap.coords](#series-treemap.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-treemap.coordinateSystem](#series-treemap.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-treemap.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-treemap.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
[缩放](#series-treemap.roam) 的极限控制，通过 `min` 和 `max` 限制最小和最大的缩放值。
...
将节点的 `id`（即 [series-treemap.data.id](#series-treemap.data.id)）映射到颜色列表中。`id` 是用户指定的，这样能够使得，在treemap 通过 setOption 变化数值时，同一 `id` 映射到同一颜色，保持一致性。参见 [例子](http://localhost/echarts-website/examples/zh/editor.html?c=treemap-obama&edit=1&reset=1)。
...
参见 [例子](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/treemap-borderColor&edit=1&reset=1)，注意其中红色的区块中的子矩形其实是更深层级，和其他用白色缝隙区分的矩形不是在一个层级。所以，红色区块中矩形的分割线的颜色，我们在 `borderColorSaturation` 中设置为『加了饱和度变化的红颜色』，以示区别。
...
参见这个[例子](http://localhost/echarts-website/examples/zh/editor.html?c=treemap-disk&edit=1&reset=1)，最顶层级用颜色区分，分成了『红』『绿』『蓝』等大块。每个颜色块中是下一个层级，使用颜色的饱和度来区分（参见 `colorSaturation`）。最外层的矩形边界是『白色』，下层级的矩形边界是当前区块颜色加上饱和度处理（参见 `borderColorSaturation`）。
...
treemap 默认把第一个维度（Array 第一项）映射到『面积』上。而如果想表达更多信息，用户可以把其他的某一个维度（[series-treemap.visualDimension](#series-treemap.viusalDimension)），映射到其他的『视觉元素』上，比如颜色明暗等。参见[例子](http://localhost/echarts-website/examples/zh/editor.html?c=treemap-obama&edit=1&reset=1)中，legend选择 `Growth`时的状态。
...
\[警告\]: 此 URL 字符串直接被使用，并未在内部做其他净化处理（sanitization） 如果他们来自于“不受信任”的来源，必须考虑 **安全风险**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。

...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#thumbnail.calendarIndex) 或 [calendarId](#thumbnail.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#thumbnail.matrixIndex) 或 [matrixId](#thumbnail.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [thumbnail.coordinateSystemUsage](#thumbnail.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#thumbnail.coordinateSystem) 上布局本 `thumbnail`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [thumbnail](#thumbnail)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [thumbnail.coords](#thumbnail.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[thumbnail.coordinateSystem](#thumbnail.coordinateSystem)。
...
当 [coordinateSystemUsage](#thumbnail.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#thumbnail.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

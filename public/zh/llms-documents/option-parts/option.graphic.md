...
文本限制宽度，用于提供 [overflow](#graphic.elements-text.style.overflow) 的参考。

#### elements-text.style.overflow
- **Type**: `string`

当文本内容超出 [width](#graphic.elements-text.style.width) 时的文本显示策略，取值：`'break'`, `'breakAll'`, `'truncate'`, `'none'`。

*   `'break'`: 尽可能保证完整的单词不被截断(类似 `CSS` 中的 `word-break: break-word;`)
*   `'breakAll'`: 可在任意字符间断行
*   `'truncate'`: 截断文本屏显示 '...'，可以使用 [ellipsis](#graphic.elements-text.style.ellipsis) 来自定义省略号的显示
*   `'none'`: 不换行

#### elements-text.style.ellipsis
- **Type**: `string`

当 [overflow](#graphic.elements-text.style.overflow) 设置为 `'truncate'` 时生效，默认为 `...`。
...
[left](#graphic.elements-compoundPath.left) 和 [right](#graphic.elements-compoundPath.right) 只有一个可以生效。

如果指定 [left](#graphic.elements-compoundPath.left) 或 [right](#graphic.elements-compoundPath.right)，则 [shape](#graphic.elements-compoundPath.shape) 里的 `x`、`cx` 等定位属性不再生效。
...
[top](#graphic.elements-compoundPath.top) 和 [bottom](#graphic.elements-compoundPath.bottom) 只有一个可以生效。

如果指定 [top](#graphic.elements-compoundPath.top) 或 [bottom](#graphic.elements-compoundPath.bottom)，则 [shape](#graphic.elements-compoundPath.shape) 里的 `y`、`cy` 等定位属性不再生效。

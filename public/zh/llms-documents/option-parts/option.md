...
当图例数量过多时，可以使用 [滚动图例（垂直）](http://localhost/echarts-website/examples/zh/editor.html?c=pie-legend&edit=1&reset=1) 或 [滚动图例（水平）](http://localhost/echarts-website/examples/zh/editor.html?c=radar2&edit=1&reset=1)，参见：[legend.type](#legend.type)
...
> **注意：** 一般来说，axisPointer 的具体配置项会配置在各个轴中（如 [xAxis.axisPointer](#xAxis.axisPointer)）或者 `tooltip` 中（如 [tooltip.axisPointer](#tooltip.axisPointer)）。
...
**注意：** 如果发现此时 tooltip 效果不良好，可设置 [tooltip.triggerOn](#tooltip.triggerOn) 为 `'none'`（于是效果为：手指按住按钮则显示 tooltip，松开按钮则隐藏 tooltip），或者 [tooltip.alwaysShowContent](#tooltip.alwaysShowContent) 为 `true`（效果为 tooltip 一直显示）。

参见[例子](http://localhost/echarts-website/examples/zh/editor.html?c=line-tooltip-touch&edit=1&reset=1)。
...
**Tip:** geo 区域的颜色也可以被 map series 所控制，参见 [series-map.geoIndex](#series-map.geoIndex)。
...
建议把 [series-parallel.lineStyle.width](#series-parallel.lineStyle.width) 设为 `0.5`（或更小）， 可能显著改善性能。
...
[image](#graphic.elements-image), [text](#graphic.elements-text), [circle](#graphic.elements-circle), [sector](#graphic.elements-sector), [ring](#graphic.elements-ring), [polygon](#graphic.elements-polygon), [polyline](#graphic.elements-polyline), [rect](#graphic.elements-rect), [line](#graphic.elements-line), [bezierCurve](#graphic.elements-bezierCurve), [arc](#graphic.elements-arc), [group](#graphic.elements-group),
...
除此以外，可以以 transform 的方式对图形进行平移、旋转、缩放， 参见：[position](#graphic.elements.position)、[rotation](#graphic.elements.rotation)、[scale](#graphic.elements.scale)、[origin](#graphic.elements.origin)。
...
灵活利用 echarts 图表和坐标系的组合，以及 API，可以实现更丰富的效果。 [在日历中使用文字](http://localhost/echarts-website/examples/zh/editor.html?c=calendar-lunar&edit=1&reset=1)、 [在日历中放置饼图](http://localhost/echarts-website/examples/zh/editor.html?c=calendar-pie&edit=1&reset=1)
...
矩阵坐标系（`matrix`）还可作为盒布局系统，布局各个系列、其他坐标系（如 `grid`（即笛卡尔坐标系）、`geo`、`polar` 等）和普通组件（如 `legend`、`dataZoom` 等）。该特性支持在表格中布局 [微型图表](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)，或使用类似 [CSS grid layout](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-grid-layout&edit=1&reset=1) 的布局方式进行排版。目前，所有系列和组件均可在 `matrix` 中布局。`matrix` 也可纯用作文字数据表。
...
以及其他 **微型图** 的例子：[矩阵中的微型柱状图和地图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-mini-bar-data-collection&edit=1&reset=1)。
...
示例：[graph NPM](http://localhost/echarts-website/examples/zh/editor.html?c=graph-npm&edit=1&reset=1)、[graph Webkit dep](http://localhost/echarts-website/examples/zh/editor.html?c=graph-webkit-dep&edit=1&reset=1)。
...
关于 `dataset` 的详情，请参见[教程](http://localhost/echarts-website/handbook/zh/concepts/dataset)。
...
`drill down` 功能即点击后才展示子层级。 设置了 [leafDepth](#series-treemap.leafDepth) 后，下钻（`drill down`）功能开启。
...
也支持多个 `series` 在同一个坐标系中，参见 [例子](http://localhost/echarts-website/examples/zh/editor.html?c=boxplot-multi&edit=1&reset=1)。
...
`custom` 系列往往需要定义 [series.encode](#series-custom.encode)，主要用于指明 `data` 的哪些维度映射到哪些数轴上。从而，echarts 能根据这些维度的值的范围，画出合适的数轴刻度。 同时，encode.tooltip 和 encode.label 也可以被指定，指明默认的 tooltip 和 label 显示什么内容。[series.dimensions](#series-custom.dimensions) 也可以被指定，指明显示在 tooltip 中的维度名称，或者维度的类型。
...
是否是暗黑模式，默认会根据背景色 [backgroundColor](#backgroundColor) 的亮度自动设置。 如果是设置了容器的背景色而无法判断到，就可以使用该配置手动指定，echarts 会根据是否是暗黑模式调整文本等的颜色。
...
`'lighter'` 也是比较常见的一种混合模式，该模式下图形数量集中的区域会颜色叠加成高亮度的颜色（白色）。常常能起到突出该区域的效果。见示例 [全球飞行航线](http://localhost/echarts-website/examples/zh/editor.html?c=lines-airline)
...
注意，这个参数实际影响的是『展示』，而非用户输入的时间值的解析。 关于用户输入的时间值（例如 `1491339540396`, `'2013-01-04'` 等）的解析，参见 [date 中时间相关部分](#series-line.data)。

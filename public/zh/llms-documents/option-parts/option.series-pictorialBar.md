...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](#grid) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](#series-pictorialBar.xAxisIndex) 和 [yAxisIndex](#series-pictorialBar.yAxisIndex) 或者 [xAxisId](#series-pictorialBar.xAxisId) 和 [yAxisId](#series-pictorialBar.yAxisId) 指定所使用的坐标轴。
...
也参见 [series-pictorialBar.coordinateSystemUsage](#series-pictorialBar.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-pictorialBar.coordinateSystem) 上布局本 `series-pictorialBar`。
...
*   `'box'`：**（不适用于 [series-pictorialBar](#series-pictorialBar)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-pictorialBar.coords](#series-pictorialBar.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-pictorialBar.coordinateSystem](#series-pictorialBar.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-pictorialBar.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-pictorialBar.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
*   当 [symbolRepeat](#series-pictorialBar.symbolRepeat) 为 `true`/`'fixed'` 的时候： 这里设置的 `symbolMargin` 只是个参考值，真正最后的图形间隔，是根据 [symbolRepeat](#series-pictorialBar.symbolRepeat)、`symbolMargin`、[symbolBoundingData](#series-pictorialBar.symbolBoundingData) 综合计算得到。
*   当 [symbolRepeat](#series-pictorialBar.symbolRepeat) 为一个固定数值的时候： 这里设置的 `symbolMargin` 无效。
...
标注名称。定义后可在 [label formatter](#series-pictorialBar.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-pictorialBar.markPoint.data.x) 和 [y](#series-pictorialBar.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标注名称。定义后可在 [label formatter](#series-pictorialBar.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-pictorialBar.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

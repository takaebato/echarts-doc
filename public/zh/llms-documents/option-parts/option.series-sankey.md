...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#series-sankey.calendarIndex) 或 [calendarId](#series-sankey.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#series-sankey.matrixIndex) 或 [matrixId](#series-sankey.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-sankey.coordinateSystemUsage](#series-sankey.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-sankey.coordinateSystem) 上布局本 `series-sankey`。
...
*   `'box'`：**（不适用于 [series-sankey](#series-sankey)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-sankey.coords](#series-sankey.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-sankey.coordinateSystem](#series-sankey.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-sankey.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-sankey.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
`center` 一般用于控制或获取 [平移缩放（roam）](#series-sankey.roam) 时图形的位置。当平移缩放时，`center` 和 [zoom](#series-sankey.zoom) 的值会被相应改变。

注意：`center` 的值基于的是对象布局的原始坐标系而非画布的坐标。 如果想使用画布坐标调整绘制对象的位置和大小，使用：[series-sankey.left](#series-sankey.left) / [.right](#series-sankey.right) / [.top](#series-sankey.top) / [.bottom](#series-sankey.bottom) / [.width](#series-sankey.width) / [.height](#series-sankey.height) 。
...
当 [平移缩放（roam）](#series-sankey.roam) 时，[center](#series-sankey.center) 和 `zoom` 的值会被相应改变。

## scaleLimit
- **Type**: `Object`

[缩放](#series-sankey.roam) 的极限控制，通过 `min` 和 `max` 限制最小和最大的缩放值。
...
当缩放和平移时，[center](#series-sankey.center) 和 [zoom](#series-sankey.zoom) 的值会被相应改变。
...
[缩放和平移（roam）](#series-sankey.roam) 可被鼠标触发。

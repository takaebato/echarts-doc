...
也参见示例 [geoJSON hexbin](http://localhost/echarts-website/examples/zh/editor.html?c=custom-hexbin)。
...
也参见示例 [Flight Seatmap](http://localhost/echarts-website/examples/zh/editor.html?c=geo-seatmap-flight)。
...
`center` 一般用于控制或获取 [平移缩放（roam）](#geo.roam) 时地图的位置。当平移缩放时，`center` 和 [zoom](#geo.zoom) 的值会被相应改变。

注意：`center` 的值基于的是对象布局的原始坐标系而非画布的坐标。 如果想使用画布坐标调整绘制对象的位置和大小，使用：[geo.left](#geo.left) / [.right](#geo.right) / [.top](#geo.top) / [.bottom](#geo.bottom) / [.width](#geo.width) / [.height](#geo.height) 或者 [geo.layoutCenter](#geo.layoutCenter) / [layoutSize](#geo.layoutSize)。

`center` 默认使用原始坐标（经纬度）。如果设置了 [projection](#geo.projection) 则用投影后的坐标。
...
百分比（字符串）值也可被用在 `center` 中，例如 `'30%'`，基于包围盒（由经纬度的最大最小值决定；或者，在设置了 [projection](#geo.projection) 时，由投影后得到的最大最小坐标决定）。你可以使用 `'0%'` 将包围盒的顶部或左侧对齐到视口（画布）中心，或使用 `'100%'` 将右侧或底部对齐到视口中心，或使用 `'50%'` 表示把整个图放于视口中心。
...
当 [平移缩放（roam）](#geo.roam) 时，[center](#geo.center) 和 `zoom` 的值会被相应改变。
...
[缩放](#geo.roam) 的极限控制，通过 `min` 和 `max` 限制最小和最大的缩放值。
...
当缩放和平移时，[center](#geo.center) 和 [zoom](#geo.zoom) 的值会被相应改变。
...
这个参数用于 scale 地图的长宽比。如果设置了 [projection](#geo.projection) 则无效。

地图最终计算得到的 `pixelWidth` 和 `pixelHeight` 将满足以下关系：`pixelWidth / pixelHeight = lngSpan / latSpan * aspectScale`（假设未指定 [projection](#geo.projection)，且 [preserveAspect](#geo.preserveAspect) 设为保持长宽比）。

当不使用真正的投影公式（[projection](#geo.projection)）时，GeoJSON 里的经纬度会被线性映射到像素坐标。`aspectScale` 提供了一种简单方式，用于视觉上补偿这种映射所造成的形变（由于地球是球形，经度对应的物理尺寸在高纬度地区会收缩）。例如，`aspectScale` 可以通过以下公式粗略计算：`aspectScale = Math.cos(center_latitude * Math.PI / 180)`，这与正弦投影（sinusoidal projection）相似。
...
为 geo 组件 分配的 `矩形区域` 是通过以下配置决定的： [geo.left](#geo.left) / [.right](#geo.right) / [.top](#geo.top) / [.bottom](#geo.bottom) / [.width](#geo.width) / [.height](#geo.height) / [.aspectScale](#geo.aspectScale)。
...
*   `'contain'` / `true`：保持内容的 `aspect ratio`。内容的包围盒被完整地包含在 `geo 组件矩形区域` 中，并尽可能放大以触达边界。此时可使用 [preserveAspectAlign](#geo.preserveAspectAlign) 和 [preserveAspectVerticalAlign](#geo.preserveAspectVerticalAlign) 调整位置。
*   `'cover'`：保持内容的原始 `aspect ratio`。内容的包围盒会覆盖整个 `geo 组件矩形区域`，并尽可能缩小以触达边界。此时可使用 [preserveAspectAlign](#geo.preserveAspectAlign) 和 [preserveAspectVerticalAlign](#geo.preserveAspectVerticalAlign) 调整位置。

注意：当使用 [layoutCenter](#geo.layoutCenter) 和 [layoutSize](#geo.layoutSize) 时，始终会保留宽高比，无论 `preserveAspect` 配置为何值。
...
参见 [preserveAspect](#geo.preserveAspect)。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#geo.calendarIndex) 或 [calendarId](#geo.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#geo.matrixIndex) 或 [matrixId](#geo.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [geo.coordinateSystemUsage](#geo.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#geo.coordinateSystem) 上布局本 `geo`。
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [geo.coords](#geo.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[geo.coordinateSystem](#geo.coordinateSystem)。
...
当 [coordinateSystemUsage](#geo.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#geo.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

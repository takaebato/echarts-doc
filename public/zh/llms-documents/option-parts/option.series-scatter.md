...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](#grid) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](#series-scatter.xAxisIndex) 和 [yAxisIndex](#series-scatter.yAxisIndex) 或者 [xAxisId](#series-scatter.xAxisId) 和 [yAxisId](#series-scatter.yAxisId) 指定所使用的坐标轴。
...
*   `'polar'`
    ...
    布局在一个 [极坐标系](#polar) 中。当一个 ECharts 实例中存在多个极坐标系时，须通过 [polarIndex](#series-scatter.polarIndex) 或 [polarId](#series-scatter.polarId) 指定所使用的极坐标系。
    

*   `'geo'`
    
    布局在一个 [地理坐标系](#geo) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](#series-scatter.geoIndex) 或 [geoId](#series-scatter.geoId) 指定所使用的地理坐标系。
    

*   `'singleAxis'`
    
    布局在一个 [单轴坐标系](#singleAxis) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](#series-scatter.singleAxisIndex) 或 [singleAxisId](#series-scatter.singleAxisId) 指定所使用的单轴标系。
    

*   `'calendar'`
    
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#series-scatter.calendarIndex) 或 [calendarId](#series-scatter.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#series-scatter.matrixIndex) 或 [matrixId](#series-scatter.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-scatter.coordinateSystemUsage](#series-scatter.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-scatter.coordinateSystem) 上布局本 `series-scatter`。
...
*   `'box'`：**（不适用于 [series-scatter](#series-scatter)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-scatter.coords](#series-scatter.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-scatter.coordinateSystem](#series-scatter.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-scatter.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-scatter.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
标注名称。定义后可在 [label formatter](#series-scatter.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-scatter.markPoint.data.x) 和 [y](#series-scatter.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标注名称。定义后可在 [label formatter](#series-scatter.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-scatter.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

...
*   `'geo'`
    ...
    布局在一个 [地理坐标系](#geo) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](#series-pie.geoIndex) 或 [geoId](#series-pie.geoId) 指定所使用的地理坐标系。
    
    参见示例 [地理坐标系中的饼图](http://localhost/echarts-website/examples/zh/editor.html?c=map-iceland-pie&edit=1&reset=1)。
    

*   `'calendar'`
    
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#series-pie.calendarIndex) 或 [calendarId](#series-pie.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#series-pie.matrixIndex) 或 [matrixId](#series-pie.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-pie.coordinateSystemUsage](#series-pie.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-pie.coordinateSystem) 上布局本 `series-pie`。
...
*   `'box'`：**（不适用于 [series-pie](#series-pie)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-pie.coords](#series-pie.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-pie.coordinateSystem](#series-pie.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-pie.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
[例子：地理坐标系中的饼图](http://localhost/echarts-website/examples/zh/editor.html?c=map-iceland-pie&edit=1&reset=1)

> 注：当 [coordinateSystemUsage](#series-pie.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
如果不需要开启该策略，例如[圆环图](http://localhost/echarts-website/examples/zh/editor.html?c=pie-doughnut)这个例子中需要强制所有标签放在中心位置，可以将该值设为 `false`。
...
*   `'center'`
    ...
    在饼图中心位置。见[圆环图示例](http://localhost/echarts-website/examples/zh/editor.html?c=pie-doughnut)
...
标注名称。定义后可在 [label formatter](#series-pie.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-pie.markPoint.data.x) 和 [y](#series-pie.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标注名称。定义后可在 [label formatter](#series-pie.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-pie.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](#grid) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](#series-custom.xAxisIndex) 和 [yAxisIndex](#series-custom.yAxisIndex) 或者 [xAxisId](#series-custom.xAxisId) 和 [yAxisId](#series-custom.yAxisId) 指定所使用的坐标轴。
...
*   `'polar'`
    ...
    布局在一个 [极坐标系](#polar) 中。当一个 ECharts 实例中存在多个极坐标系时，须通过 [polarIndex](#series-custom.polarIndex) 或 [polarId](#series-custom.polarId) 指定所使用的极坐标系。
    

*   `'geo'`
    
    布局在一个 [地理坐标系](#geo) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](#series-custom.geoIndex) 或 [geoId](#series-custom.geoId) 指定所使用的地理坐标系。
    

*   `'singleAxis'`
    
    布局在一个 [单轴坐标系](#singleAxis) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](#series-custom.singleAxisIndex) 或 [singleAxisId](#series-custom.singleAxisId) 指定所使用的单轴标系。
    

*   `'calendar'`
    
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#series-custom.calendarIndex) 或 [calendarId](#series-custom.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#series-custom.matrixIndex) 或 [matrixId](#series-custom.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-custom.coordinateSystemUsage](#series-custom.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-custom.coordinateSystem) 上布局本 `series-custom`。
...
*   `'box'`：**（不适用于 [series-custom](#series-custom)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-custom.coords](#series-custom.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-custom.coordinateSystem](#series-custom.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-custom.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-custom.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
当需要采用 barLayout 的时候，比如向柱状图上附加些东西，可以用这个方法得到 layout 信息。 参见 [例子](http://localhost/echarts-website/examples/zh/editor.html?c=custom-bar-trend)。
...
`transition` 效果参考 [例子](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/custom-transition-simple&edit=1&reset=1)。
...
动画缓动。不同的缓动效果可以参考 [缓动示例](http://localhost/echarts-website/examples/zh/editor.html?c=line-easing)。
...
也参见这个 [例子](http://localhost/echarts-website/examples/zh/editor.html?c=custom-spiral-race&edit=1&reset=1)。
...
参见例子： [icons](http://localhost/echarts-website/examples/zh/editor.html?c=custom-calendar-icon) 和 [shapes](http://localhost/echarts-website/examples/zh/editor.html?c=custom-gantt-flight)。
...
文本限制宽度，用于提供 [overflow](#series-custom.renderItem.return_text.style.overflow) 的参考。

##### renderItem.return_text.style.overflow
- **Type**: `string`

当文本内容超出 [width](#series-custom.renderItem.return_text.style.width) 时的文本显示策略，取值：`'break'`, `'breakAll'`, `'truncate'`, `'none'`。

*   `'break'`: 尽可能保证完整的单词不被截断(类似 `CSS` 中的 `word-break: break-word;`)
*   `'breakAll'`: 可在任意字符间断行
*   `'truncate'`: 截断文本屏显示 '...'，可以使用 [ellipsis](#series-custom.renderItem.return_text.style.ellipsis) 来自定义省略号的显示
*   `'none'`: 不换行

##### renderItem.return_text.style.ellipsis
- **Type**: `string`

当 [overflow](#series-custom.renderItem.return_text.style.overflow) 设置为 `'truncate'` 时生效，默认为 `...`。
...
参见 [diffChildrenByName](#series-custom.renderItem.return_compoundPath.diffChildrenByName)。

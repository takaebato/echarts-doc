...
参见这个 [示例](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/timeline-dynamic-series&edit=1&reset=1)。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#timeline.calendarIndex) 或 [calendarId](#timeline.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#timeline.matrixIndex) 或 [matrixId](#timeline.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [timeline.coordinateSystemUsage](#timeline.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#timeline.coordinateSystem) 上布局本 `timeline`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [timeline](#timeline)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [timeline.coords](#timeline.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[timeline.coordinateSystem](#timeline.coordinateSystem)。
...
当 [coordinateSystemUsage](#timeline.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#timeline.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
*   `'right'`：当 [timline.orient](#timeline.orient) 为 `'vertical'` 时有效。 贴右边界放置。
...
`timeline`组件中『当前项』（`checkpoint`）的动画的缓动效果。不同的缓动效果可以参考 [缓动示例](http://localhost/echarts-website/examples/zh/view.html?c=line-easing)。

...
\[警告\]: 如果使用了 [toolbox.feature.saveAsImage](#toolbox.feature.saveAsImage)，并且没有设置 [toolbox.feature.saveAsImage.name](#toolbox.feature.saveAsImage.name)，会用 `title[0].text` 替代。这种用法是历史实现但并不推荐，应该总是显式指定[toolbox.feature.saveAsImage.name](#toolbox.feature.saveAsImage.name)；否则，不得不考虑 `title.text` 是否是个正确的文件名，以及其 **安全性**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#title.calendarIndex) 或 [calendarId](#title.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#title.matrixIndex) 或 [matrixId](#title.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [title.coordinateSystemUsage](#title.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#title.coordinateSystem) 上布局本 `title`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [title](#title)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [title.coords](#title.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[title.coordinateSystem](#title.coordinateSystem)。
...
当 [coordinateSystemUsage](#title.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#title.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

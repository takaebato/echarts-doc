...
如果 [matrix.x.data](#matrix.x.data) 没有提供，它会从 `series.data` 或者 `dataset.source` 中自动收集。

参见 [示例](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-mini-bar-data-collection&edit=1&reset=1)。
...
如果 [matrix.y.data](#matrix.y.data) 没有提供，它会从 `series.data` 或者 `dataset.source` 中自动收集。

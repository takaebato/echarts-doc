...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#parallel.calendarIndex) 或 [calendarId](#parallel.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#parallel.matrixIndex) 或 [matrixId](#parallel.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [parallel.coordinateSystemUsage](#parallel.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#parallel.coordinateSystem) 上布局本 `parallel`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [parallel](#parallel)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [parallel.coords](#parallel.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[parallel.coordinateSystem](#parallel.coordinateSystem)。
...
当 [coordinateSystemUsage](#parallel.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#parallel.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
参见 [parallel.axisExpandCenter](#parallel.axisExpandCenter) 参见 [parallel.axisExpandable](#parallel.axisExpandable)
...
[参见示例](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/parallel-all&edit=1&reset=1)
...
*   `'category'` 类目轴，适用于离散的类目数据。为该类型时类目数据可自动从 [series.data](#series.data) 或 [dataset.source](#dataset.source) 中取，或者可通过 [parallelAxis.data](#parallelAxis.data) 设置类目数据。
...
非类目轴，包括时间，数值，对数轴，`boundaryGap`是一个两个值的数组，分别表示数据最小值和最大值的延伸范围，可以直接设置数值或者相对的百分比，在设置 [min](#parallelAxis.min) 和 [max](#parallelAxis.max) 后无效。 **示例：**
...
坐标轴刻度的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](#parallelAxis.axisLabel.interval) 一样。
...
**对于时间轴（[`parallelAxis.type: 'time'`](#parallelAxis.type)）**
...
坐标轴 tooltip 设置，注意需设置 [triggerEvent](#parallelAxis.triggerEvent) 为 `true` 并启用全局 [option.tooltip](#tooltip) 组件。

...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#series-chord.calendarIndex) 或 [calendarId](#series-chord.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#series-chord.matrixIndex) 或 [matrixId](#series-chord.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-chord.coordinateSystemUsage](#series-chord.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-chord.coordinateSystem) 上布局本 `series-chord`。
...
*   `'box'`：**（不适用于 [series-chord](#series-chord)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-chord.coords](#series-chord.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-chord.coordinateSystem](#series-chord.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-chord.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-chord.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

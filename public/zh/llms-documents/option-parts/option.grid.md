...
从 `v6.0.0` 开始不推荐使用（deprecated）。参考 [grid.outerBoundsMode](#grid.outerBoundsMode)。
...
> 外边界（outer bounds）已涵盖 [grid.containLabel](#grid.containLabel) 的功能，因此 [grid.containLabel](#grid.containLabel) 被不再推荐使用（deprecated）。 `grid.containLabel: true` 等价于 `grid: {outerBoundsMode: 'same', outerBoundsContain: 'axisLabel'}`。效果上可能会有细微差别，但通常难以察觉。你可以使用以下代码来强制保持原有效果，不过一般并无此必要。
> 
> ```
...
**示例**: [outerBounds 示例](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/grid-outerBounds&edit=1&reset=1).
...
也参见 [outerBounds 示例](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/grid-outerBounds&edit=1&reset=1).
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#grid.calendarIndex) 或 [calendarId](#grid.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#grid.matrixIndex) 或 [matrixId](#grid.matrixId) 指定所使用的矩阵坐标系。
    
    参见示例 [矩阵坐标系中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
也参见 [grid.coordinateSystemUsage](#grid.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#grid.coordinateSystem) 上布局本 `grid`。
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [grid.coords](#grid.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[grid.coordinateSystem](#grid.coordinateSystem)。
...
当 [coordinateSystemUsage](#grid.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#grid.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

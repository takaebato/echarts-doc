...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](#grid) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](#series-lines.xAxisIndex) 和 [yAxisIndex](#series-lines.yAxisIndex) 或者 [xAxisId](#series-lines.xAxisId) 和 [yAxisId](#series-lines.yAxisId) 指定所使用的坐标轴。
...
*   `'geo'`
    ...
    布局在一个 [地理坐标系](#geo) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](#series-lines.geoIndex) 或 [geoId](#series-lines.geoId) 指定所使用的地理坐标系。
    

*   `'singleAxis'`
    
    布局在一个 [单轴坐标系](#singleAxis) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](#series-lines.singleAxisIndex) 或 [singleAxisId](#series-lines.singleAxisId) 指定所使用的单轴标系。
...
也参见 [series-lines.coordinateSystemUsage](#series-lines.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-lines.coordinateSystem) 上布局本 `series-lines`。
...
*   `'box'`：**（不适用于 [series-lines](#series-lines)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-lines.coords](#series-lines.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-lines.coordinateSystem](#series-lines.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-lines.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-lines.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
如果该配置项为 `true`，则可以在 [data.coords](#series-lines.data.coords) 中设置多于 2 个的顶点用来绘制多段线，在绘制路线轨迹的时候比较有用，见示例 [北京公交路线](http://localhost/echarts-website/examples/zh/editor.html?c=lines-bmap-bus)，设置为多段线后 [lineStyle.curveness](#series-lines.lineStyle.curveness) 无效。

## effect
- **Type**: `Object`

线特效的配置，见示例 [模拟迁徙](http://localhost/echarts-website/examples/zh/editor.html?c=geo-lines) 和 [北京公交路线](http://localhost/echarts-website/examples/zh/editor.html?c=lines-bmap-effect)
...
线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定。 具体支持的格式可以参考 [标线的 symbol](#series-line.markLine.data.0.symbol)
...
标注名称。定义后可在 [label formatter](#series-lines.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-lines.markPoint.data.x) 和 [y](#series-lines.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标注名称。定义后可在 [label formatter](#series-lines.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-lines.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

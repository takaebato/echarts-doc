...
对于连续型数据，自动平均切分成几段。默认为5段。 连续数据的范围需要 [max](#visualMap-piecewise.max) 和 [min](#visualMap-piecewise.min) 来指定。
...
[参见示例](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/map-visualMap-pieces&edit=1&reset=1)
...
[参见示例](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/scatter-visualMap-categories&edit=1&reset=1)
...
在 **连续型数据平均分段** 模式（即 (that is, when [visualMap-piecewise.splitNumber](#visualMap-piecewise.splitNumber) 被使用时）需指定 `min`、`max`，如果不指定，则默认为 `[0, 200]`（注意并不是默认为 series.data 的 `dataMin` 和 `dataMax`）。
...
**连续型数据平均分段** 模式（即 (that is, when [visualMap-piecewise.splitNumber](#visualMap-piecewise.splitNumber) 被使用时）需指定 `min`、`max`，如果不指定，则默认为 `[0, 200]`（注意并不是默认为 series.data 的 `dataMin` 和 `dataMax`）。
...
两端的文本，如\['High', 'Low'\]。[参见例子](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/map-visualMap-piecewise-text&edit=1&reset=1)。
...
指定用数据的『哪个维度』，映射到视觉元素上。『数据』即 [series.data](#series.data)。 可以把 [series.data](#series.data) 理解成一个二维数组，例如：
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#visualMap-piecewise.calendarIndex) 或 [calendarId](#visualMap-piecewise.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#visualMap-piecewise.matrixIndex) 或 [matrixId](#visualMap-piecewise.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [visualMap-piecewise.coordinateSystemUsage](#visualMap-piecewise.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#visualMap-piecewise.coordinateSystem) 上布局本 `visualMap-piecewise`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [visualMap-piecewise](#visualMap-piecewise)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [visualMap-piecewise.coords](#visualMap-piecewise.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[visualMap-piecewise.coordinateSystem](#visualMap-piecewise.coordinateSystem)。
...
当 [coordinateSystemUsage](#visualMap-piecewise.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#visualMap-piecewise.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

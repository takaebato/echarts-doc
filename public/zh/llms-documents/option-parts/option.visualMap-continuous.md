...
两端的文本，如 `['High', 'Low']`。[参见例子](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/map-visualMap-continuous-text&edit=1&reset=1)。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#visualMap-continuous.calendarIndex) 或 [calendarId](#visualMap-continuous.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#visualMap-continuous.matrixIndex) 或 [matrixId](#visualMap-continuous.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [visualMap-continuous.coordinateSystemUsage](#visualMap-continuous.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#visualMap-continuous.coordinateSystem) 上布局本 `visualMap-continuous`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [visualMap-continuous](#visualMap-continuous)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [visualMap-continuous.coords](#visualMap-continuous.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[visualMap-continuous.coordinateSystem](#visualMap-continuous.coordinateSystem)。
...
当 [coordinateSystemUsage](#visualMap-continuous.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#visualMap-continuous.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

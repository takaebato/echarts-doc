...
*   `'singleAxis'`
    ...
    布局在一个 [单轴坐标系](#singleAxis) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](#series-themeRiver.singleAxisIndex) 或 [singleAxisId](#series-themeRiver.singleAxisId) 指定所使用的单轴标系。
...
也参见 [series-themeRiver.coordinateSystemUsage](#series-themeRiver.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-themeRiver.coordinateSystem) 上布局本 `series-themeRiver`。
...
*   `'box'`：**（不适用于 [series-themeRiver](#series-themeRiver)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-themeRiver.coords](#series-themeRiver.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-themeRiver.coordinateSystem](#series-themeRiver.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-themeRiver.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-themeRiver.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

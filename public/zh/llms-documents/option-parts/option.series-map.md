...
也参见示例 [USA Population Estimates](http://localhost/echarts-website/examples/zh/editor.html?c=map-usa)。
...
也参见示例 [Beef Cuts](http://localhost/echarts-website/examples/zh/editor.html?c=geo-beef-cuts)。

如上所示，ECharts 也可以使用 SVG 格式的地图。详情参见：[SVG 底图](http://localhost/echarts-website/handbook/zh/how-to/component-types/geo/svg-base-map)。
...
`center` 一般用于控制或获取 [平移缩放（roam）](#series-map.roam) 时地图的位置。当平移缩放时，`center` 和 [zoom](#series-map.zoom) 的值会被相应改变。

注意：`center` 的值基于的是对象布局的原始坐标系而非画布的坐标。 如果想使用画布坐标调整绘制对象的位置和大小，使用：[series-map.left](#series-map.left) / [.right](#series-map.right) / [.top](#series-map.top) / [.bottom](#series-map.bottom) / [.width](#series-map.width) / [.height](#series-map.height) 或者 [series-map.layoutCenter](#series-map.layoutCenter) / [layoutSize](#series-map.layoutSize)。

`center` 默认使用原始坐标（经纬度）。如果设置了 [projection](#series-map.projection) 则用投影后的坐标。
...
百分比（字符串）值也可被用在 `center` 中，例如 `'30%'`，基于包围盒（由经纬度的最大最小值决定；或者，在设置了 [projection](#series-map.projection) 时，由投影后得到的最大最小坐标决定）。你可以使用 `'0%'` 将包围盒的顶部或左侧对齐到视口（画布）中心，或使用 `'100%'` 将右侧或底部对齐到视口中心，或使用 `'50%'` 表示把整个图放于视口中心。
...
**参考示例**来理解此概念：[geo roam indicator](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/geo-roam-indicator&edit=1&reset=1).
...
当 [平移缩放（roam）](#series-map.roam) 时，[center](#series-map.center) 和 `zoom` 的值会被相应改变。
...
[缩放](#series-map.roam) 的极限控制，通过 `min` 和 `max` 限制最小和最大的缩放值。
...
当缩放和平移时，[center](#series-map.center) 和 [zoom](#series-map.zoom) 的值会被相应改变。
...
[缩放和平移（roam）](#series-map.roam) 可被鼠标触发。
...
这个参数用于 scale 地图的长宽比。如果设置了 [projection](#series-map.projection) 则无效。

地图最终计算得到的 `pixelWidth` 和 `pixelHeight` 将满足以下关系：`pixelWidth / pixelHeight = lngSpan / latSpan * aspectScale`（假设未指定 [projection](#series-map.projection)，且 [preserveAspect](#series-map.preserveAspect) 设为保持长宽比）。

当不使用真正的投影公式（[projection](#series-map.projection)）时，GeoJSON 里的经纬度会被线性映射到像素坐标。`aspectScale` 提供了一种简单方式，用于视觉上补偿这种映射所造成的形变（由于地球是球形，经度对应的物理尺寸在高纬度地区会收缩）。例如，`aspectScale` 可以通过以下公式粗略计算：`aspectScale = Math.cos(center_latitude * Math.PI / 180)`，这与正弦投影（sinusoidal projection）相似。

参见 [示例](http://localhost/echarts-website/examples/zh/editor.html?c=geo-graph&edit=1&reset=1)。
...
> 注：如果发现图形被拉伸了不符合预期，参见 [preserveAspect](#series-map.preserveAspect)。
...
为 地图系列 分配的 `矩形区域` 是通过以下配置决定的： [series-map.left](#series-map.left) / [.right](#series-map.right) / [.top](#series-map.top) / [.bottom](#series-map.bottom) / [.width](#series-map.width) / [.height](#series-map.height) / [.aspectScale](#series-map.aspectScale)。
...
*   `'contain'` / `true`：保持内容的 `aspect ratio`。内容的包围盒被完整地包含在 `地图系列矩形区域` 中，并尽可能放大以触达边界。此时可使用 [preserveAspectAlign](#series-map.preserveAspectAlign) 和 [preserveAspectVerticalAlign](#series-map.preserveAspectVerticalAlign) 调整位置。
*   `'cover'`：保持内容的原始 `aspect ratio`。内容的包围盒会覆盖整个 `地图系列矩形区域`，并尽可能缩小以触达边界。此时可使用 [preserveAspectAlign](#series-map.preserveAspectAlign) 和 [preserveAspectVerticalAlign](#series-map.preserveAspectVerticalAlign) 调整位置。

注意：当使用 [layoutCenter](#series-map.layoutCenter) 和 [layoutSize](#series-map.layoutSize) 时，始终会保留宽高比，无论 `preserveAspect` 配置为何值。
...
参见 [preserveAspect](#series-map.preserveAspect)。
...
**参见示例：** [geo roam indicator](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/geo-roam-indicator&edit=1&reset=1).
...
*   `'geo'`
    ...
    布局在一个 [地理坐标系](#geo) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](#series-map.geoIndex) 或 [geoId](#series-map.geoId) 指定所使用的地理坐标系。
...
也参见 [series-map.coordinateSystemUsage](#series-map.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-map.coordinateSystem) 上布局本 `series-map`。
...
*   `'box'`：**（不适用于 [series-map](#series-map)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-map.coords](#series-map.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-map.coordinateSystem](#series-map.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-map.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-map.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
参见这个 [示例](http://localhost/echarts-website/examples/zh/editor.html?c=dataset-series-layout-by&theme=lite)
...
标注名称。定义后可在 [label formatter](#series-map.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-map.markPoint.data.x) 和 [y](#series-map.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标注名称。定义后可在 [label formatter](#series-map.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-map.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

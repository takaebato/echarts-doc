...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](#grid) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](#series-bar.xAxisIndex) 和 [yAxisIndex](#series-bar.yAxisIndex) 或者 [xAxisId](#series-bar.xAxisId) 和 [yAxisId](#series-bar.yAxisId) 指定所使用的坐标轴。
...
*   `'polar'`
    ...
    布局在一个 [极坐标系](#polar) 中。当一个 ECharts 实例中存在多个极坐标系时，须通过 [polarIndex](#series-bar.polarIndex) 或 [polarId](#series-bar.polarId) 指定所使用的极坐标系。
...
也参见 [series-bar.coordinateSystemUsage](#series-bar.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-bar.coordinateSystem) 上布局本 `series-bar`。
...
*   `'box'`：**（不适用于 [series-bar](#series-bar)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-bar.coords](#series-bar.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-bar.coordinateSystem](#series-bar.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-bar.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-bar.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
是否开启实时排序，用来实现动态排序图效果，具体参见手册中[动态排序柱状图](http://localhost/echarts-website/handbook/zh/how-to/chart-types/bar/bar-race/)的教程。
...
柱条的颜色。 默认从全局调色盘 [option.color](#color) 获取颜色。
...
这是个更丰富的 `encode` 的[示例](http://localhost/echarts-website/examples/zh/view.html?c=dataset-encode1&edit=1&reset=1)：
...
*   当某维度对应于类目轴（axis.type 为 `'category'`）的时候：
    ...
    双类目轴的示例可以参考 [Github Punchcard](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-punchCard) 示例。
...
其中第一个参数 `value` 为 [data](#series-bar.data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](#series-bar.markPoint.data.x), [y](#series-bar.markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](#series-bar.markPoint.data.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](#series-bar.markPoint.data.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](#series-bar.markPoint.data.valueIndex) 或者 [valueDim](#series-bar.markPoint.data.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
标注名称。定义后可在 [label formatter](#series-bar.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
在使用 [type](#series-bar.markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

#### markPoint.data.valueDim
- **Type**: `string`

在使用 [type](#series-bar.markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-bar.markPoint.data.x) 和 [y](#series-bar.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](#series-bar.markLine.data.0.symbol)。
...
1.  通过 [x](#series-bar.markLine.data.0.x), [y](#series-bar.markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](#series-bar.markLine.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](#series-bar.markLine.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](#series-bar.markLine.data.0.valueIndex) 或者 [valueDim](#series-bar.markLine.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](#series-bar.markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markLine.data.0.valueDim
- **Type**: `string`

在使用 [type](#series-bar.markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称。定义后可在 [label formatter](#series-bar.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](#series-bar.markArea.data.0.x), [y](#series-bar.markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](#series-bar.markArea.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](#series-bar.markArea.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](#series-bar.markArea.data.0.valueIndex) 或者 [valueDim](#series-bar.markPoint.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](#series-bar.markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markArea.data.0.valueDim
- **Type**: `string`

在使用 [type](#series-bar.markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-bar.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

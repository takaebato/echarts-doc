...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](#grid) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](#series-graph.xAxisIndex) 和 [yAxisIndex](#series-graph.yAxisIndex) 或者 [xAxisId](#series-graph.xAxisId) 和 [yAxisId](#series-graph.yAxisId) 指定所使用的坐标轴。
...
*   `'polar'`
    ...
    布局在一个 [极坐标系](#polar) 中。当一个 ECharts 实例中存在多个极坐标系时，须通过 [polarIndex](#series-graph.polarIndex) 或 [polarId](#series-graph.polarId) 指定所使用的极坐标系。
    

*   `'geo'`
    
    布局在一个 [地理坐标系](#geo) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](#series-graph.geoIndex) 或 [geoId](#series-graph.geoId) 指定所使用的地理坐标系。
    

*   `'singleAxis'`
    
    布局在一个 [单轴坐标系](#singleAxis) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](#series-graph.singleAxisIndex) 或 [singleAxisId](#series-graph.singleAxisId) 指定所使用的单轴标系。
    

*   `'calendar'`
    
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#series-graph.calendarIndex) 或 [calendarId](#series-graph.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#series-graph.matrixIndex) 或 [matrixId](#series-graph.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-graph.coordinateSystemUsage](#series-graph.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-graph.coordinateSystem) 上布局本 `series-graph`。
...
*   `'box'`：**（不适用于 [series-graph](#series-graph)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-graph.coords](#series-graph.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
    *   又如，[饼图系列](#series-pie) 或 [和弦图系列](#series-chord) 可以布局在 [geo 坐标系](#geo) 或 [cartesian2d 坐标系](#grid) 中，这时其中心点是由 [series-pie.coords](#series-pie.coords) 或 [series-pie.center](#series-pie.center) 在坐标系中计算出来的。参见示例：[地理坐标系中的饼图](http://localhost/echarts-website/examples/zh/editor.html?c=map-iceland-pie&edit=1&reset=1)。

只有少数系列同时支持 `coordinateSystemUsage: 'data'` 和 `coordinateSystemUsage: 'box'`，如：[series-graph](#series-graph)、[series-map](#series-map)。例如，在 [例子 coordinateSystemUsage: 'data'](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-graph&edit=1&reset=1) 中，关系图每个节点分别布局在矩阵坐标系中，而在 [例子 coordinateSystemUsage: 'box'](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/matrix-graph-box&edit=1&reset=1) 中，关系图系列整体被布局在一个单元格中。

大多数系列仅支持 `coordinateSystemUsage: 'data'`，例如：[series-line](#series-line)、[series-bar](#series-bar)、[series-scatter](#series-scatter)。

同时，一些系列仅支持 `coordinateSystemUsage: 'box'`，例如：[series-pie](#series-pie)（示例：[地理坐标系中的饼图](http://localhost/echarts-website/examples/zh/editor.html?c=map-iceland-pie&edit=1&reset=1)）、[series-tree](#series-tree)、[series-treemap](#series-treemap)、[series-sankey](#series-sankey)。

另参考：[series-graph.coordinateSystem](#series-graph.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-graph.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。

例子：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1), [矩阵中的关系图](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/matrix-graph-box&edit=1&reset=1).

> 注：当 [coordinateSystemUsage](#series-graph.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
**参见示例**: [geo-choropleth-scatter](http://localhost/echarts-website/examples/zh/editor.html?c=geo-choropleth-scatter&reset=1&edit=1)
...
*   `'circular'` 采用环形布局，见示例 [Les Miserables](http://localhost/echarts-website/examples/zh/editor.html?c=graph-circular-layout)，布局相关的配置项见 [graph.circular](#series-graph.circular)
    
*   `'force'` 采用力引导布局，见示例 [Force](http://localhost/echarts-website/examples/zh/editor.html?c=graph-force)，布局相关的配置项见 [graph.force](#series-graph.force)
...
`center` 一般用于控制或获取 [平移缩放（roam）](#series-graph.roam) 时图形的位置。当平移缩放时，`center` 和 [zoom](#series-graph.zoom) 的值会被相应改变。

注意：`center` 的值基于的是对象布局的原始坐标系而非画布的坐标。 如果想使用画布坐标调整绘制对象的位置和大小，使用：[series-graph.left](#series-graph.left) / [.right](#series-graph.right) / [.top](#series-graph.top) / [.bottom](#series-graph.bottom) / [.width](#series-graph.width) / [.height](#series-graph.height) 。

如果 `center` 中使用绝对数值：

*   如果 [series-graph.layout](#series-graph.layout) 为 `'none'`，你需要在 [series-graph.data.x](#series-graph.data.x), [series-graph.data.y](#series-graph.data.y) 里提供每个节点的坐标。这时 `center` 中可使用绝对数值表示这种坐标。例如，
...
*   否则，如果在 [series-graph.layout](#series-graph.layout) 中设定了自动布局，坐标是自动计算的不可提前预知，于是也无法用绝对数值在 `center` 中设定合适的坐标。
...
**参考示例**来理解此概念：[graph roam indicator](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/graph-roam-indicator&edit=1&reset=1).
...
当 [平移缩放（roam）](#series-graph.roam) 时，[center](#series-graph.center) 和 `zoom` 的值会被相应改变。
...
[缩放](#series-graph.roam) 的极限控制，通过 `min` 和 `max` 限制最小和最大的缩放值。
...
当缩放和平移时，[center](#series-graph.center) 和 [zoom](#series-graph.zoom) 的值会被相应改变。
...
[缩放和平移（roam）](#series-graph.roam) 可被鼠标触发。
...
参见：[label position](http://localhost/echarts-website/examples/zh/view.html?c=doc-example/label-position)。
...
参见：[label position](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/label-position)。
...
参见：[label rotation](http://localhost/echarts-website/examples/zh/editor.html?c=bar-label-rotation)。
...
> **注意：**`series.data.tooltip` 仅在 [tooltip.trigger](#tooltip.trigger) 为 `'item'` 时有效。  
...
模板变量有 `{a}`, `{b}`，`{c}`，`{d}`，`{e}`，分别表示系列名，数据名，数据值等。 在 [trigger](#tooltip.trigger) 为 `'axis'` 的时候，会有多个系列的数据，此时可以通过 `{a0}`, `{a1}`, `{a2}` 这种后面加索引的方式表示系列的索引。 不同图表类型下的 `{a}`，`{b}`，`{c}`，`{d}` 含义不一样。 其中变量`{a}`, `{b}`, `{c}`, `{d}`在不同图表类型下代表数据含义为：
...
\[警告\]: tooltip 是用 HTML 实现的（除非 [tooltip.renderMode](#tooltip.renderMode) 设为 `richText`）。允许用此方式定制 HTML。传入 HTML 前须要对其内容进行正确转义。 使用时必须考虑 **安全风险**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
\[警告\]: tooltip 是用 HTML 实现的（除非 [tooltip.renderMode](#tooltip.renderMode) 设为 `richText`）。允许用此方式定制 toolbox 外壳的 CSS text。 如果此 CSS text 来自于“不受信任”的来源，必须考虑 **安全风险**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
标注名称。定义后可在 [label formatter](#series-graph.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-graph.markPoint.data.x) 和 [y](#series-graph.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
初始动画的缓动效果。不同的缓动效果可以参考 [缓动示例](http://localhost/echarts-website/examples/zh/editor.html?c=line-easing)。
...
也可以看[该示例](http://localhost/echarts-website/examples/zh/editor.html?c=bar-animation-delay)
...
4.  如果是笛卡尔坐标系的话，也可以通过只指定 `xAxis` 或者 `yAxis` 来实现 X 轴或者 Y 轴为某值的标线，见示例 [scatter-weight](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-weight)
...
标注名称。定义后可在 [label formatter](#series-graph.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
4.  如果是笛卡尔坐标系的话，也可以通过只指定 `xAxis` 或者 `yAxis` 来实现 X 轴或者 Y 轴为某值的标域，见示例 [scatter-weight](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-weight)
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-graph.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
> 注：如果发现图形被拉伸了不符合预期，参见 [preserveAspect](#series-graph.preserveAspect)。
...
为 graph 系列 分配的 `矩形区域` 是通过以下配置决定的： [series-graph.left](#series-graph.left) / [.right](#series-graph.right) / [.top](#series-graph.top) / [.bottom](#series-graph.bottom) / [.width](#series-graph.width) / [.height](#series-graph.height)。
...
*   `'contain'` / `true`：保持内容的 `aspect ratio`。内容的包围盒被完整地包含在 `graph 系列矩形区域` 中，并尽可能放大以触达边界。此时可使用 [preserveAspectAlign](#series-graph.preserveAspectAlign) 和 [preserveAspectVerticalAlign](#series-graph.preserveAspectVerticalAlign) 调整位置。
*   `'cover'`：保持内容的原始 `aspect ratio`。内容的包围盒会覆盖整个 `graph 系列矩形区域`，并尽可能缩小以触达边界。此时可使用 [preserveAspectAlign](#series-graph.preserveAspectAlign) 和 [preserveAspectVerticalAlign](#series-graph.preserveAspectVerticalAlign) 调整位置。
...
参见 [preserveAspect](#series-graph.preserveAspect)。
...
> **注意：**`series.tooltip` 仅在 [tooltip.trigger](#tooltip.trigger) 为 `'item'` 时有效。  

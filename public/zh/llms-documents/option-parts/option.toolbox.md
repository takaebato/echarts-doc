...
\[警告\]: `type` 会被用于文件的扩展名。但是实现中并没有在内部对其进行合法性校验（历史因素）。所以不要传入不合法的值；否则，需要考虑 **安全风险**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
\[警告\]: 建议总是显式指定 `name`，以及不使用“不可信任来源”的字符串。否则，须要考虑作为文件名的 **正确性** 和 **安全性**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
\[警告\]: `dataView` 面板是用 HTML 实现的，一些部分允许用户自定义格式和样式。`title` 字段会直接拼接到此 HTML 中，并没有做转义。因此传入前须要正确转义。 使用时必须考虑 **安全风险**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
\[警告\]: `dataView` 面板是用 HTML 实现的，一些部分允许用户自定义格式和样式。`optionToContent` 给出的 HTML 会直接拼接到面板 HTML 中，并没有做转义。因此传入前须要正确转义。 使用时必须考虑 **安全风险**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
\[警告\]: `dataView` 面板是用 HTML 实现的，一些部分允许用户自定义格式和样式。`lang` 中的内容会直接拼接到此 HTML 中，并没有做转义。因此传入前须要正确转义。 使用时必须考虑 **安全风险**。文档 [“安全指南”](http://localhost/echarts-website/handbook/zh/best-practices/security) 给出了安全使用建议。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](#calendar) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](#toolbox.calendarIndex) 或 [calendarId](#toolbox.calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](#matrix)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](#toolbox.matrixIndex) 或 [matrixId](#toolbox.matrixId) 指定所使用的矩阵坐标系。
...
也参见 [toolbox.coordinateSystemUsage](#toolbox.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#toolbox.coordinateSystem) 上布局本 `toolbox`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [toolbox](#toolbox)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [toolbox.coords](#toolbox.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[toolbox.coordinateSystem](#toolbox.coordinateSystem)。
...
当 [coordinateSystemUsage](#toolbox.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#toolbox.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。

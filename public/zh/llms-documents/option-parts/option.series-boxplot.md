...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](#grid) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](#series-boxplot.xAxisIndex) 和 [yAxisIndex](#series-boxplot.yAxisIndex) 或者 [xAxisId](#series-boxplot.xAxisId) 和 [yAxisId](#series-boxplot.yAxisId) 指定所使用的坐标轴。
...
也参见 [series-boxplot.coordinateSystemUsage](#series-boxplot.coordinateSystemUsage)。
...
如何在指定的 [坐标系](#series-boxplot.coordinateSystem) 上布局本 `series-boxplot`。
...
*   `'box'`：**（不适用于 [series-boxplot](#series-boxplot)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](#grid) 可以布局在 [matrix 坐标系](#matrix) 或 [calendar 坐标系](#calendar) 中，这时其布局矩形是由 [series-boxplot.coords](#series-boxplot.coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](http://localhost/echarts-website/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-boxplot.coordinateSystem](#series-boxplot.coordinateSystem)。
...
当 [coordinateSystemUsage](#series-boxplot.coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](#series-boxplot.coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
但是 ECharts 也额外提供了简单的 [原始数据处理函数](https://github.com/apache/echarts/tree/master/extension/dataTool)，如这个 [例子](http://localhost/echarts-website/examples/zh/editor.html?c=boxplot-light-velocity&edit=1&reset=1) 使用了`echarts.dataTool.prepareBoxplotData` 来进行简单的数据统计。
...
标注名称。定义后可在 [label formatter](#series-boxplot.markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](#series-boxplot.markPoint.data.x) 和 [y](#series-boxplot.markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标注名称。定义后可在 [label formatter](#series-boxplot.markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](#series-boxplot.markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

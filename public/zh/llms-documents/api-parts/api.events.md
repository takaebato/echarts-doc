...
参见 [ECharts 中的事件和行为](http://localhost/echarts-website/handbook/zh/concepts/event)
...
**ACTION:** [legendToggleSelect](#action.legend.legendToggleSelect) 切换图例选中状态后的事件。
...
**ACTION:** [legendSelect](#action.legend.legendSelect) 图例选中后的事件。
...
**ACTION:** [legendUnSelect](#action.legend.legendUnSelect) 图例取消选中后的事件。
...
**ACTION:** [legendAllSelect](#action.legend.legendAllSelect) 图例全选后的事件。
...
**ACTION:** [legendInverseSelect](#action.legend.legendInverseSelect) 图例反选后的事件。
...
**ACTION:** [legendscroll](#action.legend.legendScroll) 图例滚动事件。
...
**ACTION:** [selectDataRange](#action.dataRange.selectDataRange) 视觉映射组件中，`range` 值改变后触发的事件。
...
**ACTION:** [timelineChange](#action.timeline.timelineChange) 时间轴中的时间点改变后的事件。
...
**ACTION:** [timelinePlayChange](#action.timeline.timelinePlayChange) 时间轴中播放状态的切换事件。
...
**ACTION:** [restore](#action.toolbox.restore) 重置 option 事件。
...
[geo](option.html#geo) 中地图区域切换选中状态的事件。
...
[geo](option.html#{componentTypeFull}) 中地图区域选中后的事件。

使用`dispatchAction`可触发此事件，用户点击不会触发此事件（用户点击事件请使用 [geoselectchanged](#events.geoselectchanged)）。
...
[geo](option.html#geo) 中地图区域取消选中后的事件。

...
*   从 [Apache ECharts 官网下载界面](http://localhost/echarts-website/zh/download.html) 获取官方源码包后构建。
...
你也可以直接进入 [ECharts Gallery](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/getting-started) 中查看编辑示例
...
最简单的更改全局样式的方式，是直接采用颜色主题（theme）。例如，在 [示例集合](http://localhost/echarts-website/examples/zh/index.html) 中，可以选择 “Theme”，直接看到采用主题的效果。
...
[入门示例](#getting-started)中的数据是在初始化后 `setOption` 中直接填入的，但是很多时候可能数据需要异步加载后再填入。Apache EChartsTM 中实现异步数据的更新非常简单，在图表初始化后不管任何时候只要通过 jQuery 等工具异步获取数据后通过 `setOption` 填入数据和配置项就行。
...
答： 关于标签的显示 [label.formatter](option.html#series.label.formatter)，现在支持引用特定维度的值，例如：
...
Apache EChartsTM 5 开始支持了“数据转换”（ data transform ）功能。在 echarts 中，“数据转换” 这个词指的是，给定一个已有的“数据集”（[dataset](option.html#dataset)）和一个“转换方法”（[transform](option.html#dataset.transform)），echarts 能生成一个新的“数据集”，然后可以使用这个新的“数据集”绘制图表。这些工作都可以声明式地完成。
...
例如，在内置的 "boxplot" transform 中，除了 boxplot 系列所需要的 data 外，离群点（ outlier ）也会被生成，并且可以用例如散点图系列显示出来。例如，[example](http://localhost/echarts-website/examples/zh/editor.html?c=boxplot-light-velocity&edit=1&reset=1)。
...
*   [聚集](http://localhost/echarts-website/examples/zh/editor.html?c=data-transform-aggregate&edit=1&reset=1)
*   [直方图](http://localhost/echarts-website/examples/zh/editor.html?c=bar-histogram&edit=1&reset=1)
*   [简单聚类](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-clustering&edit=1&reset=1)
*   [线性回归线](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-linear-regression&edit=1&reset=1)
*   [指数回归线](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-exponential-regression&edit=1&reset=1)
*   [对数回归线](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-logarithmic-regression&edit=1&reset=1)
*   [多项式回归线](http://localhost/echarts-website/examples/zh/editor.html?c=scatter-polynomial-regression&edit=1&reset=1)
...
This is another [media query example](http://localhost/echarts-website/examples/zh/editor.html?c=doc-example/bar-media-timeline&edit=1&reset=1).
...
此外，ECharts 还提供了 [visualMap 组件](option.html#visualMap) 来提供通用的视觉映射。`visualMap` 组件中可以使用的视觉元素有：  
...
**更多的例子参见：[custom examples](http://localhost/echarts-website/examples/zh/index.html#chart-type-custom)**
...
在这个例子中，『维度1』和『维度2』对应到 X 轴，`dataZoom` 组件控制 X 轴的缩放。假如在缩放的过程中，某个 dataItem 的『维度1』超出了 X 轴的范围，『维度2』还在 X 轴的范围中，那么只要设置 `dataZoom.filterMode = 'weakFilter'`，这个 dataItem 就不会被过滤掉，从而还能够使用 `renderItem` 绘制图形（可以使用上面提到过的 `echarts.graphic.clipRectByRect` 把图形绘制成被坐标系剪裁过的样子）。参见上面提到过的例子：[Profile](http://localhost/echarts-website/examples/zh/editor.html?c=custom-profile)
...
自定义系列能支持使用 [SVG PathData](http://www.w3.org/TR/SVG/paths.html#PathData) 定义矢量路径。从而可以使用矢量图工具中做出的图形。参见：[path](option.html#series-custom.renderItem.return_path)，以及例子：[icons](http://localhost/echarts-website/examples/zh/editor.html?c=custom-calendar-icon) 和 [shapes](http://localhost/echarts-website/examples/zh/editor.html?c=custom-gantt-flight)。

  

**更多的自定义系列的例子参见：[custom examples](http://localhost/echarts-website/examples/zh/index.html#chart-type-custom)**
...
其他一些例子： [Map Labels](http://localhost/echarts-website/examples/zh/editor.html?c=map-labels&edit=1&reset=1), [Pie Labels](http://localhost/echarts-website/examples/zh/editor.html?c=pie-nest&edit=1&reset=1), [Gauge](http://localhost/echarts-website/examples/zh/editor.html?c=gauge-car&edit=1&reset=1).
...
简单表格的设定，其实就是给不同行上纵向对应的文本片段设定同样的宽度就可以了。参见本文最开始的 [例子](http://localhost/echarts-website/examples/zh/view.html?c=pie-rich-text&edit=1&reset=1)。
...
Apache EChartsTM 从初始一直使用 Canvas 绘制图表（除了对 IE8- 使用 VML）。而 [ECharts v4.0](https://github.com/apache/echarts/releases) 发布了 SVG 渲染器，从而提供了一种新的选择。只须在初始化一个图表实例时，设置 [renderer 参数](api.html#echarts.init) 为 `'canvas'` 或 `'svg'` 即可指定渲染器，比较方便。
...
一般来说，Canvas 更适合绘制图形元素数量非常大（这一般是由数据量大导致）的图表（如热力图、地理坐标系或平行坐标系上的大规模线图或散点图等），也利于实现某些视觉 [特效](http://localhost/echarts-website/examples/zh/editor.html?c=lines-bmap-effect)。但是，在不少场景中，SVG 具有重要的优势：它的内存占用更低（这对移动端尤其重要）、渲染性能略高、并且用户使用浏览器内置的缩放功能时不会模糊。
...
注：除了某些特殊的渲染可能依赖 Canvas：如[炫光尾迹特效](option.html#series-lines.effect)、[带有混合效果的热力图](http://localhost/echarts-website/examples/zh/editor.html?c=heatmap-bmap)等，绝大部分功能 SVG 都是支持的。
...
*   如果 [在线自定义构建 ECharts](http://localhost/echarts-website/zh/builder.html)，则需要勾上页面下方的 “SVG 渲染”。
...
[庖丁解牛](http://localhost/echarts-website/examples/zh/editor.html?c=geo-beef-cuts) | [内脏数据](http://localhost/echarts-website/examples/zh/editor.html?c=geo-organ) | [航班选座](http://localhost/echarts-website/examples/zh/editor.html?c=geo-seatmap-flight) | [地图](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-map) | [散点图](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-scatter-simple) | [路径图](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-lines) | [交通](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-traffic)
...
参见例子 [roam](option.html#geo.roam)、[SVG 地图](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-map)。
...
此外，使用 [series-map](option.html#series-map) 时，也可以用 [visualMap 组件](option.html#visualMap) 为具名元素赋予样式。参见例子 [庖丁解牛](http://localhost/echarts-website/examples/zh/editor.html?c=geo-beef-cuts)。
...
参见例子 [航班选座](http://localhost/echarts-website/examples/zh/editor.html?c=geo-seatmap-flight)。
...
参见例子 [Organ Visualization](http://localhost/echarts-website/examples/zh/editor.html?c=geo-organ)。
...
参见例子 [SVG 地图](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-map)。
...
标签的用法也参见示例 [Organ Visualization](http://localhost/echarts-website/examples/zh/editor.html?c=geo-organ)。
...
参见示例 [SVG Scatter](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-scatter-simple)、[SVG Lines](http://localhost/echarts-website/examples/zh/editor.html?c=geo-svg-lines)、[SVG Traffic](http://localhost/echarts-website/examples/zh/editor.html?c=geo-traffic)。
...
在看完文章之后，你可以前往 [官方示例](http://localhost/echarts-website/examples/zh/index.html#chart-type-globe) 和 [Gallery](https://gallery.echartsjs.com/explore.html#tags=echarts-gl) 去了解更多使用 GL 制作的示例，对于文章中我们没法解释到的代码，也可以前往 [GL 配置项手册](option-gl.html) 查看具体的配置项使用方法。

## 如何下载和引入 ECharts GL

为了不再增加已经很大了的 ECharts 完整版的体积，我们将 GL 作为扩展包的形式提供，和诸如水球图这样的扩展类似，如果要使用 GL 里的各种组件，只需要在引入`echarts.min.js`的基础上再引入一个`echarts-gl.min.js`。你可以从 [官网](http://localhost/echarts-website/zh/download.html) 下载最新版的 GL，然后在页面中通过标签引入：
...
可以先从 [http://localhost/echarts-website/examples/data/asset/data/life-expectancy-table.json](http://localhost/echarts-website/examples/data/asset/data/life-expectancy-table.json) 获取这份数据。
...
在 ECharts 4 中我们可以使用 dataset 组件非常方便地引入这份数据。如果对 dataset 还不熟悉的话可以看[dataset使用教程](http://localhost/echarts-website/handbook/zh/concepts/dataset)
...
当然，除了 [visualMap](option.html#visualMap) 组件，还可以利用其它的 ECharts 内置组件并且充分利用这些组件的交互效果，比如 [legend](option.html#legend)。也可以像 [三维散点图和散点矩阵结合使用](http://localhost/echarts-website/examples/zh/editor.html?c=scatter3d-scatter&gl=1) 这个例子一样实现二维和三维的系列混搭。
...
`ec-canvas` 目录下有一个 `echarts.js`，默认我们会在每次 `echarts-for-weixin` 项目发版的时候替换成最新版的 ECharts。如有必要，可以自行从 ECharts 项目中下载[最新发布版](https://github.com/apache/echarts/releases)，或者从[官网自定义构建](http://localhost/echarts-website/builder.html)以减小文件大小。
